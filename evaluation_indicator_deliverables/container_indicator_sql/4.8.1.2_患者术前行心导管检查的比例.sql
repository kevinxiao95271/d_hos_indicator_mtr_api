...
SET @num_name = '室间隔缺损术前行心导管检查患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '术前行心导管检查的患者数', '无', '反映术前检查的规范性', '第四章-室间隔缺损手术质量控制查询', NULL, 'full', NULL, NULL, '分子：术前行心导管检查的患者数。分母：同期手术的患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '术前行心导管检查的患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.8.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期室间隔缺损手术患者总数_4.8.1.2';
-- 分母 4.8.1.2.2 源于 4.8.1.1.2，须先执行 4.8.1.1_患者术前行超声心动图检查的比例.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期手术的患者总数', '无', '反映术前检查的规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.8.1.1.2（同期手术的患者总数）同源，采集源于该指标', '第四章-室间隔缺损手术质量控制查询'), NULL, 'full', NULL, NULL, '分子：术前行心导管检查的患者数。分母：同期手术的患者总数。（与4.8.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期手术的患者总数', '同期手术的患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.8.1.2.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '术前行心导管检查的患者数占同期手术的患者总数的比例', '无', '反映术前检查的规范性', '第四章-室间隔缺损手术质量控制查询', NULL, 'full', NULL, NULL, '分子：术前行心导管检查的患者数。分母：同期手术的患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.8.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
