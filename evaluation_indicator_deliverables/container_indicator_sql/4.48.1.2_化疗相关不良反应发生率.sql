-- 4.48.1.2 化疗相关不良反应发生率（儿童急性淋巴细胞白血病）
...
SET @num_name = '发生化疗相关不良反应的患儿数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '发生化疗相关不良反应的患儿数', '无', '反映化疗安全性', '第四章-儿童急性淋巴细胞白血病（初始诱导化疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：发生化疗相关不良反应的患儿数。分母：同期住院患儿总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '发生化疗相关不良反应的患儿数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.48.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期住院患儿总数_4.48.1.2';
-- 分母 4.48.1.2.2 源于 4.48.1.1.2，须先执行 4.48.1.1_诱导期化疗方案规范率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期住院患儿总数', '无', '反映化疗安全性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.48.1.1.2（同期该病种住院患儿总数）同源，采集源于该指标', '第四章-儿童急性淋巴细胞白血病（初始诱导化疗）质量控制查询'), NULL, 'full', NULL, NULL, '分子：发生化疗相关不良反应的患儿数。分母：同期住院患儿总数。（与4.48.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期该病种住院患儿总数', '同期住院患儿总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.48.1.2.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    '发生化疗相关不良反应的患儿数占同期住院患儿总数的比例', '无', '反映化疗安全性', '第四章-儿童急性淋巴细胞白血病（初始诱导化疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：发生化疗相关不良反应的患儿数。分母：同期住院患儿总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.48.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
