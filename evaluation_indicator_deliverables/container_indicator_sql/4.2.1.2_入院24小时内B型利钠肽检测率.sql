-- 4.2.1.2 入院24小时内B型利钠肽检测率（心力衰竭）
-- 分母 4.2.1.2.2 源于 4.2.1.1.2，须先执行 4.2.1.1_心脏功能评估率.sql
...
SET @numerator_kpi_name = '入院24小时内BNP检测的心力衰竭患者数';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', NULL, @data_source_id, @data_source_param,
    '入院24小时内进行B型利钠肽检测的心力衰竭患者人数', '无', '评价心力衰竭患者评估的规范性、及时性',
    '第四章-心力衰竭质量控制查询', NULL, 'full', NULL, NULL, '分子：入院24小时内BNP检测数。分母：同期心力衰竭患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '入院24小时内BNP检测数', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.2.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

SET @denominator_kpi_name = '同期心力衰竭患者总数_4.2.1.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '分子：入院24小时内BNP检测数。分母：同期心力衰竭患者总数（与4.2.1.1.2一致）。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期心力衰竭患者总数', '同期心力衰竭患者总数_BNP'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.2.1.2.2"}]'),
...
SET @kpi_name = '入院24小时内B型利钠肽检测率';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @data_source_id, @data_source_param,
    '入院24小时内B型利钠肽检测的心力衰竭患者数占同期心力衰竭患者总数的比例', '无', '评价心力衰竭患者评估的规范性、及时性',
    '第四章-心力衰竭质量控制查询', NULL, 'full', NULL, NULL, '分子：入院24小时内BNP检测数。分母：同期心力衰竭患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @numerator_kpi_id, @denominator_kpi_id, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.2.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL
);
