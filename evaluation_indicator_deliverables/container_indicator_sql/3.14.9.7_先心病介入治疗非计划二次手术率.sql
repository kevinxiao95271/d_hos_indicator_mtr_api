-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.14.9.7（与CSV文件中的编码完全一致）
-- 指标名称：先心病介入治疗非计划二次手术率
-- 创建日期：2026-02-04
-- ===========================================
...
-- 创建分子指标（3.14.9.7.1）
SET @numerator_kpi_name = '行先心病介入治疗后非计划二次手术的患者数';
SET @numerator_category_code = '3.14.9.7.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '行先心病介入治疗后非计划二次手术的患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '先心病介入治疗专病表查询', NULL, '分子：行先心病介入治疗后非计划二次手术的患者数。分母：同期行先心病介入治疗的患者总数。', '无', '无', NULL,
...
SET @denominator_category_code = '3.14.9.7.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.9.7）
SET @kpi_name = '先心病介入治疗非计划二次手术率';
SET @category_code = '3.14.9.7';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '行先心病介入治疗后非计划二次手术的患者数，占同期行先心病介入治疗的患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '先心病介入治疗专病表查询', NULL, '分子：行先心病介入治疗后非计划二次手术的患者数。分母：同期行先心病介入治疗的患者总数。', '百分比（%）', '无', NULL,
