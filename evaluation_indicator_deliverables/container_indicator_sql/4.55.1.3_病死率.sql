...
SET @num_name = 'PTE死亡人数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '某病种的死亡人数', '无', '评价医疗质量的重要指标', '第四章-急性肺血栓栓塞症质量控制查询', NULL, 'full', NULL, NULL, '分子：PTE死亡人数。分母：同期急性肺血栓栓塞症患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '某病种的死亡人数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.55.1.3.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期PTE出院总人数_4.55.1.3';
-- 分母 4.55.1.3.2 源于 4.55.1.1.2，须先执行 4.55.1.1_入院氧合评估完成率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期急性肺血栓栓塞症患者总数', '无', '评价医疗质量的重要指标', IF(@source_kpi_id IS NOT NULL, '与指标编码4.55.1.1.2（同期急性肺血栓栓塞症患者总数）同源，采集源于该指标', '第四章-急性肺血栓栓塞症质量控制查询'), NULL, 'full', NULL, NULL, '分子：PTE死亡人数。分母：同期急性肺血栓栓塞症患者总数。（与4.55.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期急性肺血栓栓塞症患者总数', '同期某病种出院总人数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.55.1.3.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    'PTE死亡人数占同期急性肺血栓栓塞症患者总数的比例', '无', '评价医疗质量的重要指标', '第四章-急性肺血栓栓塞症质量控制查询', NULL, 'full', NULL, NULL, '分子：PTE死亡人数。分母：同期急性肺血栓栓塞症患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.55.1.3"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
