...
-- 创建分子指标（3.14.11.2.1）
SET @numerator_kpi_name = '行CIED植入术发生心脏压塞的患者数';
SET @numerator_category_code = '3.14.11.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '行CIED植入术发生心脏压塞的患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '心律失常介入治疗专病表查询', NULL, '分子：行CIED植入术发生心脏压塞的患者数。分母：同期行CIED植入术的患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否发生心脏压塞_CIED', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.11.2.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.11.2）
SET @kpi_name = 'CIED植入术心脏压塞发生率';
SET @category_code = '3.14.11.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '行CIED植入术发生心脏压塞的患者数，占同期行CIED植入术的患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '心律失常介入治疗专病表查询', NULL, '分子：行CIED植入术发生心脏压塞的患者数。分母：同期行CIED植入术的患者总数。', '百分比（%）', '无', NULL,
