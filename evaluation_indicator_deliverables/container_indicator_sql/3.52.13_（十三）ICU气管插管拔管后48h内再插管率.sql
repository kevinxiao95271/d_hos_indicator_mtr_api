-- ===========================================
-- 0352 专病表指标注册脚本
-- 指标编码：3.52.13（十三）ICU气管插管拔管后48h内再插管率
-- 数据源：重症医学专病表查询
-- ===========================================
...
SET @numerator_kpi_name = 'ICU气管插管计划拔管后48h内再插管例数';
SET @numerator_category_code = '3.52.13.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, '{"p_date_type":"out"}',
    'ICU气管插管计划拔管后48h内再插管例数', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映对ICU患者脱机拔管指征的把握能力',
    '重症医学专病表查询', NULL, 'full', '分子：ICU气管插管计划拔管后48h内再插管例数。分母：同期ICU患者气管插管拔管总例数。不包括非计划气管插管拔管后再插管。', '无', '无', NULL, '无', '无', '无',
    '出院日期', '出院科室名称', '是否ICU气管插管计划拔管后48h内再插管', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期ICU患者气管插管拔管总例数_再插管';
SET @denominator_category_code = '3.52.13.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, '{"p_date_type":"out"}',
    '同期ICU患者气管插管拔管总例数', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映对ICU患者脱机拔管指征的把握能力',
    '重症医学专病表查询', NULL, 'full', '分子：ICU气管插管计划拔管后48h内再插管例数。分母：同期ICU患者气管插管拔管总例数。气管插管拔管不包括非计划气管插管拔管。', '无', '无', NULL, '无', '无', '无',
    '出院日期', '出院科室名称', '是否同期ICU患者气管插管计划拔管', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = '（十三）ICU气管插管拔管后48h内再插管率';
SET @category_code = '3.52.13';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @data_source_id, '{"p_date_type":"out"}',
    '气管插管计划拔管后48h内再插管例数占同期ICU患者气管插管拔管总例数的比例', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映对ICU患者脱机拔管指征的把握能力',
    '重症医学专病表查询', NULL, 'full', '本指标中气管插管拔管不包括非计划气管插管拔管。不包括非计划气管插管拔管后再插管。', NULL, NULL, '百分比（%）', '无', '无', '无',
