...
-- 创建分子指标（3.14.7.4.1）
SET @numerator_kpi_name = '行主动脉瓣手术后发生脑卒中的患者数';
SET @numerator_category_code = '3.14.7.4.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '行主动脉瓣手术后发生脑卒中的患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '主动脉瓣手术专病表查询', NULL, '分子：行主动脉瓣手术后发生脑卒中的患者数。分母：同期行主动脉瓣手术的患者总数。', '无', '无', NULL,
...
SET @denominator_category_code = '3.14.7.4.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.7.4）
SET @kpi_name = '主动脉瓣手术后脑卒中发生率';
SET @category_code = '3.14.7.4';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '行主动脉瓣手术后发生脑卒中的患者数，占同期行主动脉瓣手术的患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '主动脉瓣手术专病表查询', NULL, '分子：行主动脉瓣手术后发生脑卒中的患者数。分母：同期行主动脉瓣手术的患者总数。', '百分比（%）', '无', NULL,
