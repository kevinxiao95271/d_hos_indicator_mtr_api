...
SET @numerator_kpi_name = '住院期间行心电图检查慢阻肺患者数';
SET @numerator_category_code = '3.7.10.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '住院期间行心电图检查慢阻肺患者数', '《呼吸系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价慢阻肺急性加重患者检查的规范性',
    '慢阻肺急性加重专病表查询', NULL, '分子：住院期间行心电图检查慢阻肺患者数。分母：同期住院慢阻肺患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否住院期间行心电图检查', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期住院慢阻肺患者总数';
SET @denominator_category_code = '3.7.10.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期住院慢阻肺患者总数', '《呼吸系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价慢阻肺急性加重患者检查的规范性',
    '慢阻肺急性加重专病表查询', NULL, '分子：住院期间行心电图检查慢阻肺患者数。分母：同期住院慢阻肺患者总数。', '无', '无', NULL,
...
SET @kpi_name = '慢阻肺急性加重患者住院期间心电图检查比例';
SET @category_code = '3.7.10';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，住院期间行心电图检查的慢阻肺急性加重患者数占同期住院慢阻肺患者总数的比例', '《呼吸系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价慢阻肺急性加重患者检查的规范性',
    '慢阻肺急性加重专病表查询', NULL, '分子：住院期间行心电图检查慢阻肺患者数。分母：同期住院慢阻肺患者总数。', '百分比（%）', '无', NULL,
