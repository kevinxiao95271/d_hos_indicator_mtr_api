...
-- 创建分子指标（3.14.9.1.1）
SET @numerator_kpi_name = '行先心病介入治疗成功的患者数';
SET @numerator_category_code = '3.14.9.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '行先心病介入治疗成功的患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '先心病介入治疗专病表查询', NULL, '分子：行先心病介入治疗成功的患者数。先心病包括房间隔缺损（ASD）、室间隔缺损（VSD）、动脉导管未闭（PDA）及肺动脉瓣狭窄（PS）。治疗成功指通过介入手段治疗先心病后，达到治愈原先天性畸形或明显改善其血流动力学，且未发生严重并发症。分母：同期行先心病介入治疗的患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否先心病介入治疗成功', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.9.1.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.9.1）
SET @kpi_name = '先心病介入治疗成功率';
SET @category_code = '3.14.9.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '行先心病介入治疗成功的患者数，占同期行先心病介入治疗的患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '先心病介入治疗专病表查询', NULL, '分子：行先心病介入治疗成功的患者数。先心病包括房间隔缺损（ASD）、室间隔缺损（VSD）、动脉导管未闭（PDA）及肺动脉瓣狭窄（PS）。治疗成功指通过介入手段治疗先心病后，达到治愈原先天性畸形或明显改善其血流动力学，且未发生严重并发症。分母：同期行先心病介入治疗的患者总数。', '百分比（%）', '无', NULL,
