...
SET @numerator_kpi_name = '进行深静脉血栓（DVT）预防的ICU住院患者数';
SET @numerator_category_code = '3.1.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '进行深静脉血栓（DVT）预防的ICU住院患者数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU对深静脉血栓的预防情况',
    '重症医学专病表查询', NULL, '分子：进行深静脉血栓（DVT）预防的ICU住院患者数。分母：同一统计周期内ICU收治患者总数。', '无', '无', NULL,
...
SET @kpi_name = 'ICU深静脉血栓（DVT）预防率';
SET @category_code = '3.1.6';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '进行深静脉血栓（DVT）预防的ICU患者数占同期ICU收治患者总数的比例', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU对深静脉血栓的预防情况',
    '重症医学专病表查询', NULL, '分子：进行深静脉血栓（DVT）预防的ICU住院患者数。分母：同一统计周期内ICU收治患者总数。', '百分比（%）', '无', NULL,
