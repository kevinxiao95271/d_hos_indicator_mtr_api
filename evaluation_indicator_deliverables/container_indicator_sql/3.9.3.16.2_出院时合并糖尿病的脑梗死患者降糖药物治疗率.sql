-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.9.3.16.2（与CSV文件中的编码完全一致）
-- 指标名称：出院时合并糖尿病的脑梗死患者降糖药物治疗率
-- 创建日期：2026-02-05
-- ===========================================
...
-- 创建分子指标（3.9.3.16.2.1）
SET @numerator_kpi_name = '出院时给予降糖药物治疗的 合并糖尿病的脑梗死患者数';
SET @numerator_category_code = '3.9.3.16.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '出院时给予降糖药物治疗的 合并糖尿病的脑梗死患者数', '', '',
    '脑梗死专病表查询', NULL, '', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否出院时给予降糖药物治疗的 合并糖尿病的脑梗死', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建分母指标（3.9.3.16.2.2）
SET @denominator_kpi_name = '同期合并糖尿病的住院脑梗死患者总数';
SET @denominator_category_code = '3.9.3.16.2.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期合并糖尿病的住院脑梗死患者总数', '', '',
    '脑梗死专病表查询', NULL, '', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期合并糖尿病的脑梗死患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
-- 创建比率指标（3.9.3.16.2）
SET @kpi_name = '出院时合并糖尿病的脑梗死患者降糖药物治疗率';
SET @category_code = '3.9.3.16.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，出院时给予降糖药物治疗的合并糖尿病的脑梗死患者数占同期合并糖尿病的住院脑梗死患者总数的比例。', '', '',
