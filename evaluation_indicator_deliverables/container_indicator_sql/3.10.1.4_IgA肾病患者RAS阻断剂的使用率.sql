...
SET @numerator_kpi_name = '使用RAS阻断剂的IgA肾病患者数';
SET @numerator_category_code = '3.10.1.4.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '使用RAS阻断剂的IgA肾病患者数', '《肾病专业医疗质量控制指标（2021年版）》。', '评价IgA肾病患者RAS阻断剂使用的规范性',
    'IgA肾病专病表查询', NULL, '分子：使用RAS阻断剂的IgA肾病患者数。分母：同期适合使用RAS阻断剂的IgA肾病患者数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否使用RAS阻断剂', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期适合使用RAS阻断剂的IgA肾病患者数';
SET @denominator_category_code = '3.10.1.4.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期适合使用RAS阻断剂的IgA肾病患者数', '《肾病专业医疗质量控制指标（2021年版）》。', '评价IgA肾病患者RAS阻断剂使用的规范性',
    'IgA肾病专病表查询', NULL, '分子：使用RAS阻断剂的IgA肾病患者数。分母：同期适合使用RAS阻断剂的IgA肾病患者数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否适合使用RAS阻断剂', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = 'IgA肾病患者RAS阻断剂的使用率';
SET @category_code = '3.10.1.4';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '适合使用RAS阻断剂的IgA肾病患者中使用RAS阻断剂的比例', '《肾病专业医疗质量控制指标（2021年版）》。', '评价IgA肾病患者RAS阻断剂使用的规范性',
    'IgA肾病专病表查询', NULL, '分子：使用RAS阻断剂的IgA肾病患者数。分母：同期适合使用RAS阻断剂的IgA肾病患者数。', '百分比（%）', '无', NULL,
