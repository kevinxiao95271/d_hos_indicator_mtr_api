...
SET @num_name = '脑梗死静脉溶栓指征评估患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '住院期间进行静脉溶栓指征评估的患者数', '无', '反映脑梗死患者溶栓治疗的规范性、安全性', '第四章-脑梗死（首次住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：住院期间进行静脉溶栓指征评估的患者数。分母：同期住院脑梗死患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '住院期间进行静脉溶栓指征评估的患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.9.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期住院脑梗死患者总数_4.9.1.2';
-- 分母 4.9.1.2.2 源于 4.9.1.1.2，须先执行 4.9.1.1_患者入院神经功能缺损评估率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期住院脑梗死患者总数', '无', '反映脑梗死患者溶栓治疗的规范性、安全性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.9.1.1.2（同期住院脑梗死患者总数）同源，采集源于该指标', '第四章-脑梗死（首次住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：住院期间进行静脉溶栓指征评估的患者数。分母：同期住院脑梗死患者总数。（与4.9.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期住院脑梗死患者总数', '同期住院脑梗死患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.9.1.2.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
SET @kpi_name = '静脉溶栓指征评估率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '住院期间进行静脉溶栓指征评估的患者数占同期住院脑梗死患者总数的比例', '无', '反映脑梗死患者溶栓治疗的规范性、安全性', '第四章-脑梗死（首次住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：住院期间进行静脉溶栓指征评估的患者数。分母：同期住院脑梗死患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.9.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
