...
-- 创建分子指标
SET @numerator_kpi_name = '术后发生早期器官功能不全的手术例数';
SET @numerator_category_code = '5.1.12.3.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '自体器官移植术后发生早期器官功能不全的手术例数占同期自体器官移植术总例数的比例。', '《自体器官移植技术临床应用管理规范（2022 版）》《自体器官移植技术临床应用质量控制指标（2022 版）》。', '反映自体器官移植技术治疗的安全性，是反映医院自体器官移植技术医疗质量的重要结果性指标之一。',
    '自体器官移植技术数据源', NULL, '无', '百分比（%）', '病案首页。', NULL,
    '出院日期', '出院科室名称', '是否完成术后早期器官功能不全发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.12.3.1.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期自体器官移植术总例数';
SET @denominator_category_code = '5.1.12.3.1.2';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.12.3.1.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '术后早期器官功能不全发生率';
SET @category_code = '5.1.12.3.1';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.12.3.1"}]'), NULL, NULL, NULL, NULL
);
