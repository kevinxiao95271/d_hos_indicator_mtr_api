-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.14.1.15（与CSV文件中的编码完全一致）
-- 指标名称：急性STEMI患者出院后30天内非计划再入院率
-- 创建日期：2026-02-04
-- ===========================================
...
-- 创建分子指标（3.14.1.15.1）
SET @numerator_kpi_name = '出院后30天内非计划再入院的急性STEMI患者数';
SET @numerator_category_code = '3.14.1.15.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '出院后30天内，原先无计划再入院，而因任何原因再次入院的急性STEMI患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院STEMI诊疗的整体水平',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：出院后30天内，原先无计划再入院，而因任何原因再次入院的急性STEMI患者数。再次入院的医疗机构不限。分母：同期出院的急性STEMI患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否出院后30天内非计划再入院', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建分母指标（3.14.1.15.2）
SET @denominator_kpi_name = '同期出院的急性STEMI患者总数';
SET @denominator_category_code = '3.14.1.15.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '出院的急性STEMI患者总数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院STEMI诊疗的整体水平',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：出院后30天内，原先无计划再入院，而因任何原因再次入院的急性STEMI患者数。再次入院的医疗机构不限。分母：同期出院的急性STEMI患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期出院的急性STEMI患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
-- 创建比率指标（3.14.1.15）
SET @kpi_name = '急性STEMI患者出院后30天内非计划再入院率';
SET @category_code = '3.14.1.15';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '出院后30天内，原先无计划再入院，而因任何原因再次入院的急性STEMI患者数，占同期出院的急性STEMI患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院STEMI诊疗的整体水平',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：出院后30天内，原先无计划再入院，而因任何原因再次入院的急性STEMI患者数。再次入院的医疗机构不限。分母：同期出院的急性STEMI患者总数。', '百分比（%）', '无', NULL,
