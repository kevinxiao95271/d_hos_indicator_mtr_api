-- 4.39.3.1 病例上报例数（腮腺肿瘤）
SET @data_source_id = (SELECT ID FROM DATASOURCE WHERE DATASOURCE_NAME = '单病种病例上报查询' AND INVALID = 0 LIMIT 1);
SET @category_id = (SELECT ID FROM KPI_CATEGORY WHERE CATEGORY_NAME LIKE '%腮腺肿瘤%' AND INVALID = 0 LIMIT 1);
SET @create_user = 'system';
SET @data_source_param = '{"p_disease_code":"4.39"}';

SET @kpi_name = '腮腺肿瘤病例上报例数';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', NULL, @data_source_id, @data_source_param,
    '报告至国家单病种质量监测平台的腮腺肿瘤病例数', '《单病种系统接口规范》', '反映单病种病例向国家平台上报情况',
    ...
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.39.3.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL
);
