-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.14.3.7（与CSV文件中的编码完全一致）
-- 指标名称：心力衰竭患者住院期间心脏再同步化治疗（CRT）使用率
-- 创建日期：2026-02-04
-- ===========================================
...
-- 创建分子指标（3.14.3.7.1）
SET @numerator_kpi_name = '住院期间给予CRT治疗的心力衰竭患者数';
SET @numerator_category_code = '3.14.3.7.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '住院期间给予CRT治疗的心力衰竭患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院救治心力衰竭患者的情况',
    '心力衰竭专病表查询', NULL, '分子：住院期间给予CRT治疗的心力衰竭患者数。分母：同期心力衰竭患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否住院期间给予CRT治疗', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建比率指标（3.14.3.7）
SET @kpi_name = '心力衰竭患者住院期间心脏再同步化治疗（CRT）使用率';
SET @category_code = '3.14.3.7';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '住院期间给予CRT治疗的心力衰竭患者数，占同期心力衰竭患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院救治心力衰竭患者的情况',
    '心力衰竭专病表查询', NULL, '分子：住院期间给予CRT治疗的心力衰竭患者数。分母：同期心力衰竭患者总数。', '百分比（%）', '无', NULL,
