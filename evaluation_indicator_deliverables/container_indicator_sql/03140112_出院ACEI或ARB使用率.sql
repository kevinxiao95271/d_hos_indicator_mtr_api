-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.14.1.12（与CSV文件中的编码完全一致）
-- 指标名称：急性STEMI患者出院血管紧张素转换酶抑制剂（ACEI）或血管紧张素Ⅱ受体拮抗剂（ARB）使用率
-- 创建日期：2026-02-04
-- ===========================================
...
-- 创建分子指标（3.14.1.12.1）
SET @numerator_kpi_name = '出院使用ACEI/ARB的急性STEMI患者数';
SET @numerator_category_code = '3.14.1.12.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '出院使用ACEI或ARB的急性STEMI患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价STEMI二级预防情况',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：出院使用ACEI或ARB的急性STEMI患者数。分母：同期急性STEMI患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否出院使用ACEI或ARB', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.1.12.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.1.12）
SET @kpi_name = '急性STEMI患者出院血管紧张素转换酶抑制剂（ACEI）或血管紧张素Ⅱ受体拮抗剂（ARB）使用率';
SET @category_code = '3.14.1.12';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '出院使用ACEI或ARB的急性STEMI患者数，占同期急性STEMI患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价STEMI二级预防情况',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：出院使用ACEI或ARB的急性STEMI患者数。分母：同期急性STEMI患者总数。', '百分比（%）', '无', NULL,
