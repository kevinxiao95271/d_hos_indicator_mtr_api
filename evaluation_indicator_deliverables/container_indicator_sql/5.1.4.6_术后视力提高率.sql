...
-- 创建分子指标
SET @numerator_kpi_name = '术后视力提高的例数';
SET @numerator_category_code = '5.1.4.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '术后视力提高是指同种异体角膜移植术后，视力提高视力表两行及以上。术后视力提高率是指术后视力提高的例数占同期同种异体角膜移植总例数的比例。', '《同种异体角膜移植技术临床应用管理规范（2022 版）》 《同种异体角膜移植技术临床应用质量控制指标（2022 版）》。', '反映同种异体角膜移植术后视力改善情况。',
    '同种异体角膜移植技术数据源', NULL, '分子：是指在患者住院过程中完成同种异体角膜移植术，术后视力提高视力表两行及以上的例数。 分母：是指同期出院患者中完成同种异体角膜移植的总例数。', '百分比（%）', '病历资料。', NULL,
    '出院日期', '出院科室名称', '是否完成术后视力提高', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.4.6.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期同种异体角膜移植总例数';
SET @denominator_category_code = '5.1.4.6.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成术后视力提高', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.4.6.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '术后视力提高率';
SET @category_code = '5.1.4.6';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.4.6"}]'), NULL, NULL, NULL, NULL
);
