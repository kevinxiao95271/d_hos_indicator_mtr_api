-- 医疗安全指标 2.3.29 注册脚本（分子+分母+比率）
-- 指标编码：2.3.29 / 2.3.29.1 / 2.3.29.2
-- 指标名称：（二十九）ICU血管内导管相关血流感染（CRBSI）发生率
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.29"}';

SET @numerator_kpi_name = 'ICU血管内导管相关血流感染发生例数';
SET @numerator_category_code = '2.3.29.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    'ICU血管内导管相关血流感染发生例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：ICU血管内导管相关血流感染发生例数；分母：同期ICU患者血管内导管留置总天数。',
    'ICU血管内导管相关血流感染发生例数', '无', NULL, '无',
...
SET @denominator_kpi_name = '同期ICU患者血管内导管留置总天数';
SET @denominator_category_code = '2.3.29.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '同期ICU患者血管内导管留置总天数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分母：同期ICU患者血管内导管留置总天数。',
    '同期ICU患者血管内导管留置总天数', '无', NULL, '无',
...
SET @kpi_name = '（二十九）ICU血管内导管相关血流感染（CRBSI）发生率';
SET @category_code = '2.3.29';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 4, 'decrease', 'ratio', @data_source_id, @data_source_param,
    'ICU患者血管内导管相关血流感染发生率是指ICU患者血管内导管相关血流感染发生例数占同期ICU患者血管内导管留置总天数的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：ICU血管内导管相关血流感染发生例数；分母：同期ICU患者血管内导管留置总天数。',
    '（二十九）ICU血管内导管相关血流感染（CRBSI）发生率', '例/千导管日', NULL, '无',
