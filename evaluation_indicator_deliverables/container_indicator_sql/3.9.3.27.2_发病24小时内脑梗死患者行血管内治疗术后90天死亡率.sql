-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.9.3.27.2（与CSV文件中的编码完全一致）
-- 指标名称：发病24小时内脑梗死患者行血管内治疗术后90天死亡率
-- 创建日期：2026-02-05
-- ===========================================
...
-- 创建分子指标（3.9.3.27.2.1）
SET @numerator_kpi_name = '发病24小时内脑梗死患者行血管内治疗术后90天死亡人数';
SET @numerator_category_code = '3.9.3.27.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '发病24小时内脑梗死患者行血管内治疗术后90天死亡人数', '', '',
    '脑梗死专病表查询', NULL, '术后随访包括电话随访、网络随访、门诊随访、再次住院。术后住院期间死亡以病案首页信息为依据。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否发病24小时内脑梗死患者行血管内治疗术后90天死亡', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建比率指标（3.9.3.27.2）
SET @kpi_name = '发病24小时内脑梗死患者行血管内治疗术后90天死亡率';
SET @category_code = '3.9.3.27.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，发病24小时内脑梗死患者行血管内治疗者中，术后住院期间、术后90天死亡的患者所占比例。', '', '',
    '脑梗死专病表查询', NULL, '术后随访包括电话随访、网络随访、门诊随访、再次住院。术后住院期间死亡以病案首页信息为依据。', '百分比（%）', '无', NULL,
