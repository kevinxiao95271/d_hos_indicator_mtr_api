...
SET @numerator_kpi_name = 'ICU住院患者使用抗菌药物前病原学检验标本送检病例数';
SET @numerator_category_code = '3.1.5.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    'ICU住院患者使用抗菌药物前病原学检验标本送检病例数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU抗菌药物使用的规范性',
    '重症医学专病表查询', NULL, '分子：ICU住院患者使用抗菌药物前病原学检验标本送检病例数。分母：同一统计周期内ICU住院患者使用抗菌药物治疗病例总数。', '无', '无', NULL,
...
SET @denominator_kpi_name = '同一统计周期内ICU住院患者使用抗菌药物治疗病例总数';
SET @denominator_category_code = '3.1.5.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同一统计周期内ICU住院患者使用抗菌药物治疗病例总数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU抗菌药物使用的规范性',
    '重症医学专病表查询', NULL, '分子：ICU住院患者使用抗菌药物前病原学检验标本送检病例数。分母：同一统计周期内ICU住院患者使用抗菌药物治疗病例总数。', '无', '无', NULL,
...
SET @kpi_name = 'ICU抗菌药物治疗前病原学送检率';
SET @category_code = '3.1.5';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '以治疗为目的使用抗菌药物的ICU住院患者，使用抗菌药物前病原学检验标本送检病例数占同期使用抗菌药物治疗病例总数的比例', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU抗菌药物使用的规范性',
    '重症医学专病表查询', NULL, '分子：ICU住院患者使用抗菌药物前病原学检验标本送检病例数。分母：同一统计周期内ICU住院患者使用抗菌药物治疗病例总数。', '百分比（%）', '无', NULL,
