...
-- 创建分子指标（3.14.9.6.1）
SET @numerator_kpi_name = '行先心病介入治疗后给予输血的患者数';
SET @numerator_category_code = '3.14.9.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '行先心病介入治疗后给予输血的患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '先心病介入治疗专病表查询', NULL, '分子：行先心病介入治疗后给予输血的患者数。输血指先心病介入治疗术后由于各种原因输注红细胞、血浆及血小板。分母：同期行先心病介入治疗的患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否术后给予输血', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.9.6.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.9.6）
SET @kpi_name = '先心病介入治疗输血率';
SET @category_code = '3.14.9.6';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '行先心病介入治疗后给予输血的患者数，占行先心病介入治疗的患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '先心病介入治疗专病表查询', NULL, '分子：行先心病介入治疗后给予输血的患者数。输血指先心病介入治疗术后由于各种原因输注红细胞、血浆及血小板。分母：同期行先心病介入治疗的患者总数。', '百分比（%）', '无', NULL,
