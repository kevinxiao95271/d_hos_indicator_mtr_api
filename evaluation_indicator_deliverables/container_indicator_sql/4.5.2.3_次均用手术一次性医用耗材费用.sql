...
-- 单病种应上报资源消耗指标注册脚本
-- 指标编码：4.5.2.3（以CSV为准）
-- 指标名称：主动脉瓣置换术次均用手术一次性医用耗材费用
-- 创建日期：2026-02
-- 说明：分母 4.5.2.3.2 与 4.5.1.1.2（同期手术患者的总例数）统计口径一致，采集模式为「源于其他指标」，须先执行 4.5.1.1_术前评估完成率.sql
-- ===================================
...
SET @category_id = (
    ...
    WHERE CATEGORY_NAME LIKE '%主动脉瓣置换术%' AND INVALID = 0
    LIMIT 1
);

SET @create_user = 'system';
SET @data_source_param = '{"p_date_type":"out","p_disease_code":"AVR"}';

SET @numerator_kpi_name = '主动脉瓣置换术手术耗材费用之和';
SET @numerator_category_code = '4.5.2.3.1';
...
SET @denominator_kpi_name = '同期主动脉瓣置换术手术患者总例数';
SET @denominator_category_code = '4.5.2.3.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.5.1.1.2（同期手术患者的总例数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL,
    '分子：手术用一次性医用耗材费用之和。分母：同期手术患者总例数（与4.5.1.1.2一致）。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期手术患者的总例数', '计数'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @kpi_name = '主动脉瓣置换术次均用手术一次性医用耗材费用';
SET @category_code = '4.5.2.3';
CALL sp_kpi_item_create(
