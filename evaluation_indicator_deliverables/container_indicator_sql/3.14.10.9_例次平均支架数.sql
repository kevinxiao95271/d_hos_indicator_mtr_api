...
-- 创建分子指标（3.14.10.9.1）
SET @numerator_kpi_name = '植入冠脉总支架数';
SET @numerator_category_code = '3.14.10.9.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '植入冠脉总支架数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价支架使用合理性',
    '冠心病介入治疗专病表查询', NULL, '分子：植入冠脉总支架数。分母：同期接受冠脉介入治疗的总病例数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '植入支架数', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建分母指标（3.14.10.9.2）
SET @denominator_kpi_name = '同期接受冠脉介入治疗的总病例数';
SET @denominator_category_code = '3.14.10.9.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期接受冠脉介入治疗的总病例数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价支架使用合理性',
    '冠心病介入治疗专病表查询', NULL, '分子：植入冠脉总支架数。分母：同期接受冠脉介入治疗的总病例数。', '无', '无', NULL,
...
-- 创建比率指标（3.14.10.9）
SET @kpi_name = '例次平均支架数';
SET @category_code = '3.14.10.9';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'integer', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '平均每例次手术中置入支架的个数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价支架使用合理性',
    '冠心病介入治疗专病表查询', NULL, '分子：植入冠脉总支架数。分母：同期接受冠脉介入治疗的总病例数。', '个', '无', NULL,
