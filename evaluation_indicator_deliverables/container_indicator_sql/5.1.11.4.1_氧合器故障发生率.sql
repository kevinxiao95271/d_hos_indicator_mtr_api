...
-- 创建分子指标
SET @numerator_kpi_name = '氧合器故障例次数';
SET @numerator_category_code = '5.1.11.4.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '氧合器故障包括①血栓;②血浆渗漏;③漏血;④氧合欠佳等需氧合器更换或终止ECMO。', '《体外膜肺氧合（ECMO）技术临床应用管理规范（2022 版）》《体外膜肺氧合（ECMO）技术临床应用质量控制指标（2022 版）》。', '反映ECMO技术治疗的安全性，是反映医院ECMO技术医疗质量的重要结果性指标之一。',
    '体外膜肺氧合（ECMO）技术数据源', NULL, '无', '百分比（%）', '病案首页。', NULL,
    '出院日期', '出院科室名称', '是否完成氧合器故障发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.11.4.1.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期 ECMO 应用氧合器例次数';
SET @denominator_category_code = '5.1.11.4.1.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '氧合器故障包括①血栓;②血浆渗漏;③漏血;④氧合欠佳等需氧合器更换或终止ECMO。', '《体外膜肺氧合（ECMO）技术临床应用管理规范（2022 版）》《体外膜肺氧合（ECMO）技术临床应用质量控制指标（2022 版）》。', '反映ECMO技术治疗的安全性，是反映医院ECMO技术医疗质量的重要结果性指标之一。',
    '体外膜肺氧合（ECMO）技术数据源', NULL, '无', '百分比（%）', '病案首页。', NULL,
    '出院日期', '出院科室名称', '是否完成ECMO应用氧合器', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.11.4.1.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '氧合器故障发生率';
SET @category_code = '5.1.11.4.1';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.11.4.1"}]'), NULL, NULL, NULL, NULL
);
