-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.9.4.8.4（与CSV文件中的编码完全一致）
-- 指标名称：合并糖尿病的颈动脉支架置入术患者出院降糖药物治疗率
-- 创建日期：2026-02-05
-- ===========================================
...
-- 创建分母指标（3.9.4.8.4.2）
SET @denominator_kpi_name = '同期合并糖尿病的颈动脉支架置入术患者总数';
SET @denominator_category_code = '3.9.4.8.4.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期合并糖尿病的颈动脉支架置入术患者总数', '', '',
    '颈动脉支架置入术专病表查询', NULL, '术后出院时给予规范化药物治疗包含：双重抗血小板药物；他汀类药物；合并高血压的颈动脉支架置入术患者术后出院时给予降压药物治疗；合并糖尿病的颈动脉支架置入术患者术后出院时给予降糖药物治疗。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期合并糖尿病的颈动脉支架置入术患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
-- 创建比率指标（3.9.4.8.4）
SET @kpi_name = '合并糖尿病的颈动脉支架置入术患者出院降糖药物治疗率';
SET @category_code = '3.9.4.8.4';
CALL sp_kpi_item_create(
