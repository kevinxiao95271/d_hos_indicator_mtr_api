...
SET @category_id = (
    ...
    WHERE CATEGORY_NAME LIKE '%癫痫持续状态%' AND INVALID = 0
    LIMIT 1
);

SET @create_user = 'system';
SET @data_source_param = '{"p_date_type":"out","p_disease_code":"CSE"}';

SET @numerator_kpi_name = '惊厥性癫痫持续状态出院患者占用总床日数';
SET @numerator_category_code = '4.16.2.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, @data_source_param,
    '惊厥性癫痫持续状态出院患者占用总床日数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：惊厥性癫痫持续状态出院患者占用总床日数。分母：同期惊厥性癫痫持续状态出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期惊厥性癫痫持续状态出院总人数';
SET @denominator_category_code = '4.16.2.1.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期惊厥性癫痫持续状态出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.16.1.1.2（同期住院惊厥性癫痫持续状态患者总数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：惊厥性癫痫持续状态出院患者占用总床日数。分母：同期惊厥性癫痫持续状态出院总人数。（与4.16.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期住院惊厥性癫痫持续状态患者总数', '计数'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @kpi_name = '惊厥性癫痫持续状态平均住院日';
SET @category_code = '4.16.2.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'decrease', 'ratio', NULL, @data_source_id, @data_source_param,
    '惊厥性癫痫持续状态出院患者占用总床日数除以同期惊厥性癫痫持续状态出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率，间接反映综合医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：惊厥性癫痫持续状态出院患者占用总床日数。分母：同期惊厥性癫痫持续状态出院总人数。', '天', '无', NULL, NULL,
