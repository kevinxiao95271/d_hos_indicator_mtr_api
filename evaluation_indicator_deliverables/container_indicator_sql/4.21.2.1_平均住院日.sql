-- ===========================================
-- 单病种应上报资源消耗指标注册脚本
-- 指标编码：4.21.2.1（以CSV为准）
-- 指标名称：哮喘（成人，急性发作，住院）平均住院日
-- 创建日期：2026-02
-- ===========================================
...
SET @data_source_param = '{"p_date_type":"out","p_disease_code":"ASTHMA_ADULT"}';

SET @numerator_kpi_name = '哮喘（成人，急性发作，住院）出院患者占用总床日数';
SET @numerator_category_code = '4.21.2.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, @data_source_param,
    '哮喘（成人，急性发作，住院）出院患者占用总床日数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：哮喘（成人，急性发作，住院）出院患者占用总床日数。分母：同期哮喘（成人，急性发作，住院）出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期哮喘（成人，急性发作，住院）出院总人数';
SET @denominator_category_code = '4.21.2.1.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期哮喘（成人，急性发作，住院）出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.21.1.1.2（同期支气管哮喘急性发作出院患者总例数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：哮喘（成人，急性发作，住院）出院患者占用总床日数。分母：同期哮喘（成人，急性发作，住院）出院总人数。（与4.21.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期支气管哮喘急性发作出院患者总数', '计数'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @kpi_name = '哮喘（成人，急性发作，住院）平均住院日';
SET @category_code = '4.21.2.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'decrease', 'ratio', NULL, @data_source_id, @data_source_param,
    '哮喘（成人，急性发作，住院）出院患者占用总床日数除以同期哮喘（成人，急性发作，住院）出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率，间接反映综合医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：哮喘（成人，急性发作，住院）出院患者占用总床日数。分母：同期哮喘（成人，急性发作，住院）出院总人数。', '天', '无', NULL, NULL,
