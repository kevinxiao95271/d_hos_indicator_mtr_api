-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.9.4.4.1（与CSV文件中的编码完全一致）
-- 指标名称：颈动脉支架置入术患者术前规范化药物治疗率
-- 创建日期：2026-02-05
-- ===========================================
...
-- 创建分子指标（3.9.4.4.1.1）
SET @numerator_kpi_name = '颈动脉支架置入术患者术前规范化药物治疗人数';
SET @numerator_category_code = '3.9.4.4.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '颈动脉支架置入术患者术前规范化药物治疗人数', '《神经系统疾病医疗质量控制指标（2020 年版）》。', '反映医疗机构对颈动脉支架置入术患者术前规范化评估的现状。',
    '颈动脉支架置入术专病表查询', NULL, '术前规范化药物治疗指使用双重抗血小板药物和他汀类药物治疗。术前双重抗血小板药物治疗指阿司匹林加氯吡格雷联合使用≥4天，或者术前使用负荷量。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否颈动脉支架置入术患者术前规范化药物治疗', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.9.4.4.1.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.9.4.4.1）
SET @kpi_name = '颈动脉支架置入术患者术前规范化药物治疗率';
SET @category_code = '3.9.4.4.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，颈动脉支架置入术患者术前规范化药物（双重抗血小板药物＋他汀类药物）治疗人数占颈动脉支架置入术患者总数的比例。', '《神经系统疾病医疗质量控制指标（2020 年版）》。', '反映医疗机构对颈动脉支架置入术患者术前规范化评估的现状。',
    '颈动脉支架置入术专病表查询', NULL, '分子：颈动脉支架置入术患者术前规范化药物治疗人数。分母：同期颈动脉支架置入术患者总数。', '百分比（%）', '无', NULL,
