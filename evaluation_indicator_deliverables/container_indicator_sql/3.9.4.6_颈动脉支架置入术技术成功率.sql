...
-- 创建分子指标（3.9.4.6.1）
SET @numerator_kpi_name = '颈动脉支架置入术技术成功人数';
SET @numerator_category_code = '3.9.4.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '颈动脉支架置入术技术成功人数', '《神经系统疾病医疗质量控制指标（2020 年版）》。 2.颈动脉支架置入术患者术前颈动脉无创影像评估率', '反映医疗机构对颈动脉支架置入术患者术前规范化评估的现状。',
    '颈动脉支架置入术专病表查询', NULL, '颈动脉支架置入术技术成功定义为术后残余狭窄≤30%且术后血流mTICI分级3级。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否颈动脉支架置入术技术成功', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.9.4.6.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.9.4.6）
SET @kpi_name = '颈动脉支架置入术技术成功率';
SET @category_code = '3.9.4.6';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，颈动脉支架置入术技术成功人数占颈动脉支架置入术患者总数的比例。', '《神经系统疾病医疗质量控制指标（2020 年版）》。 2.颈动脉支架置入术患者术前颈动脉无创影像评估率', '反映医疗机构对颈动脉支架置入术患者术前规范化评估的现状。',
    '颈动脉支架置入术专病表查询', NULL, '颈动脉支架置入术技术成功定义为术后残余狭窄≤30%且术后血流mTICI分级3级。', '百分比（%）', '无', NULL,
