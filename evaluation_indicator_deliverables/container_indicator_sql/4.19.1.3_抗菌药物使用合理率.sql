...
SET @num_name = 'CAP儿童合理应用抗菌药物患儿例数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '合理应用抗菌药物的患儿例数', '无', '反映抗菌药物合理使用情况', '第四章-社区获得性肺炎（儿童，首次住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：合理应用抗菌药物的患儿例数。分母：同期社区获得性肺炎患儿总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '合理应用抗菌药物的患儿例数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.19.1.3.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期CAP儿童患儿总数_4.19.1.3';
-- 分母 4.19.1.3.2 源于 4.19.1.1.2，须先执行 4.19.1.1_入院氧合评估完成率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期社区获得性肺炎患儿总数', '无', '反映抗菌药物合理使用情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.19.1.1.2（同期社区获得性肺炎患儿总数）同源，采集源于该指标', '第四章-社区获得性肺炎（儿童，首次住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：合理应用抗菌药物的患儿例数。分母：同期社区获得性肺炎患儿总数。（与4.19.1.1.2一致）', '无', '无', NULL, NULL,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.19.1.3.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '合理应用抗菌药物的患儿例数占同期社区获得性肺炎患儿总数的比例', '无', '反映抗菌药物合理使用情况', '第四章-社区获得性肺炎（儿童，首次住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：合理应用抗菌药物的患儿例数。分母：同期社区获得性肺炎患儿总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.19.1.3"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
