-- 医疗安全指标 2.3.6 注册脚本（分子+分母+比率）
-- 指标编码：2.3.6 / 2.3.6.1 / 2.3.6.2
-- 指标名称：（六）手术患者手术后猝死发生率
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.6"}';

SET @numerator_kpi_name = '手术患者手术后猝死发生例数';
SET @numerator_category_code = '2.3.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '手术患者手术后猝死发生例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：手术患者手术后猝死发生例数；分母：同期手术患者出院人次。',
    '手术患者手术后猝死发生例数', '无', NULL, '无',
...
SET @denominator_category_code = '2.3.6.2';
CALL sp_kpi_item_create(
...
SET @kpi_name = '（六）手术患者手术后猝死发生率';
SET @category_code = '2.3.6';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, @data_source_param,
    '手术患者手术后猝死发生率是指手术患者手术后猝死发生例数占同期手术患者出院人次的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：手术患者手术后猝死发生例数；分母：同期手术患者出院人次。',
    '（六）手术患者手术后猝死发生率', '百分比（%）', NULL, '无',
