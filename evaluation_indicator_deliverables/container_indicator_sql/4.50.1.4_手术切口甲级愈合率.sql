...
SET @num_name = '手术切口甲级愈合患者例数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '手术切口甲级愈合的患者例数', '无', '反映手术切口愈合质量', '第四章-甲状腺结节（手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：手术切口甲级愈合的患者例数。分母：同期甲状腺结节手术治疗患者总例数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '手术切口甲级愈合的患者例数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.50.1.4.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期甲状腺结节手术患者总例数_4.50.1.4';
-- 分母 4.50.1.4.2 源于 4.50.1.1.2，须先执行 4.50.1.1_甲状腺结节手术适应证合格率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期甲状腺结节手术治疗患者总例数', '无', '反映手术切口愈合质量', IF(@source_kpi_id IS NOT NULL, '与指标编码4.50.1.1.2（同期甲状腺结节手术治疗患者总例数）同源，采集源于该指标', '第四章-甲状腺结节（手术治疗）质量控制查询'), NULL, 'full', NULL, NULL, '分子：手术切口甲级愈合的患者例数。分母：同期甲状腺结节手术治疗患者总例数。（与4.50.1.1.2一致）', '无', '无', NULL, NULL,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.50.1.4.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
SET @kpi_name = '手术切口甲级愈合率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '手术切口甲级愈合的患者例数占同期甲状腺结节手术治疗患者总例数的比例', '无', '反映手术切口愈合质量', '第四章-甲状腺结节（手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：手术切口甲级愈合的患者例数。分母：同期甲状腺结节手术治疗患者总例数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.50.1.4"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
