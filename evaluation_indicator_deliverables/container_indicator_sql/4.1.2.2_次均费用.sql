...
-- 单病种应上报资源消耗指标注册脚本
-- 指标编码：4.1.2.2（以CSV为准）
-- 指标名称：急性心肌梗死（ST段抬高型，首次住院）次均费用
-- 创建日期：2026-02
-- 说明：分母 4.1.2.2.2 与 4.1.1.1.2（同期急性STEMI患者总数）统计口径一致，采集模式为「源于其他指标」，须先执行 4.1.1.1 注册脚本
-- ===========================================
...
SET @data_source_param = '{"p_date_type":"out","p_disease_code":"STEMI"}';

SET @numerator_kpi_name = '急性心肌梗死（ST段抬高型，首次住院）患者住院费用之和';
SET @numerator_category_code = '4.1.2.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'value', NULL, @data_source_id, @data_source_param,
    '所有急性心肌梗死（ST段抬高型，首次住院）患者住院费用之和', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：所有急性心肌梗死（ST段抬高型，首次住院）患者住院费用之和。分母：同期急性心肌梗死（ST段抬高型，首次住院）出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期急性心肌梗死（ST段抬高型，首次住院）出院总人数_次均费用';
SET @denominator_category_code = '4.1.2.2.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期急性心肌梗死（ST段抬高型，首次住院）出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.1.1.1.2（同期急性STEMI患者总数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL,
    '分子：所有急性心肌梗死（ST段抬高型，首次住院）患者住院费用之和。分母：同期急性心肌梗死（ST段抬高型，首次住院）出院总人数（与4.1.1.1.2一致）。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期急性STEMI患者总数', '计数'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @kpi_name = '急性心肌梗死（ST段抬高型，首次住院）次均费用';
SET @category_code = '4.1.2.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'ratio', NULL, @data_source_id, @data_source_param,
    '所有急性心肌梗死（ST段抬高型，首次住院）患者住院费用之和除以同期急性心肌梗死（ST段抬高型，首次住院）出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制，间接反映医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：所有急性心肌梗死（ST段抬高型，首次住院）患者住院费用之和。分母：同期急性心肌梗死（ST段抬高型，首次住院）出院总人数。', '元', '无', NULL, NULL,
