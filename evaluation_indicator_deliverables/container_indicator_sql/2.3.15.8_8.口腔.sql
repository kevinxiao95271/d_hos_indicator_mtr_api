-- 医疗安全指标 2.3.15.8 注册脚本（分子+分母+比率）
-- 指标编码：2.3.15.8 / 2.3.15.8.1 / 2.3.15.8.2
-- 指标名称：8.口腔
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.15.8"}';

SET @numerator_kpi_name = '口腔术后并发症发生人次数';
SET @numerator_category_code = '2.3.15.8.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '口腔术后并发症发生人次数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：口腔术后并发症发生人次数；分母：同期手术患者出院人次。',
    '口腔术后并发症发生人次数', '无', NULL, '无',
...
SET @denominator_category_code = '2.3.15.8.2';
CALL sp_kpi_item_create(
...
SET @kpi_name = '8.口腔';
SET @category_code = '2.3.15.8';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, @data_source_param,
    '口腔术后并发症发生率是指口腔手术患者手术后并发症发生例数占同期口腔手术患者出院人次的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：口腔术后并发症发生人次数；分母：同期手术患者出院人次。',
    '8.口腔', '百分比（%）', NULL, '无',
