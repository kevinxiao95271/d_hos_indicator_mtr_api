-- ===========================================
-- 单病种应上报资源消耗指标注册脚本
-- 指标编码：4.49.2.3（以CSV为准）
-- 指标名称：儿童急性早幼粒细胞白血病次均药品费用
-- 创建日期：2026-02
-- ===================================
...
SET @numerator_kpi_name = '儿童急性早幼粒细胞白血病患儿住院药品费用之和';
SET @numerator_category_code = '4.49.2.3.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'value', NULL, @data_source_id, @data_source_param,
    '该病种所有患儿住院药品费用之和', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种药品消耗情况',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：该病种所有患儿住院药品费用之和。分母：同期住院患儿总例数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '药品费用', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间","药品费用"]',
...
SET @denominator_kpi_name = '同期儿童急性早幼粒细胞白血病住院患儿总例数';
SET @denominator_category_code = '4.49.2.3.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期住院患儿总例数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种药品消耗情况',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.49.1.1.2（同期该病种住院患儿总数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：该病种所有患儿住院药品费用之和。分母：同期住院患儿总例数。（与4.49.1.1.2一致）', '无', '无', NULL, NULL,
...
SET @kpi_name = '儿童急性早幼粒细胞白血病次均药品费用';
SET @category_code = '4.49.2.3';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'ratio', NULL, @data_source_id, @data_source_param,
    '该病种所有患儿住院药品费用之和除以同期住院患儿总例数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种药品消耗情况，反映医疗成本控制',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：该病种所有患儿住院药品费用之和。分母：同期住院患儿总例数。', '元', '无', NULL, NULL,
