...
-- 创建分子指标
SET @numerator_kpi_name = '发生溶血并发症例次数';
SET @numerator_category_code = '5.1.11.5.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '溶血是指应用ECMO期间,排除出血、血液稀释等原因导致的血色素下降幅度>2g/dL,且血浆游离血红蛋白>5mg/dL。', '《体外膜肺氧合（ECMO）技术临床应用管理规范（2022 版）》《体外膜肺氧合（ECMO）技术临床应用质量控制指标（2022 版）》。', '反映ECMO技术治疗的安全性，是反映医院ECMO技术医疗质量的重要结果性指标之一。',
    '体外膜肺氧合（ECMO）技术数据源', NULL, '无', '百分比（%）', '病案首页。', NULL,
    '出院日期', '出院科室名称', '是否完成溶血发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.11.5.2.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期ECMO应用例次数';
SET @denominator_category_code = '5.1.11.5.2.2';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.11.5.2.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '溶血发生率';
SET @category_code = '5.1.11.5.2';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.11.5.2"}]'), NULL, NULL, NULL, NULL
);
