...
-- 创建分子指标
SET @numerator_kpi_name = '同一术种术中因各种原因转为其它手术方式的例数';
SET @numerator_category_code = '5.1.10.4.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同一术种实施人工智能辅助治疗技术的患者，术中因各种原因转为其它手术方式的例数占同期人工智能辅助治疗技术总例数的比例。', '《人工智能辅助治疗技术临床应用管理规范（2022 版）》 《人工智能辅助治疗技术临床应用质量控制指标（2022 版）》。', '反映医院人工智能辅助治疗技术水平及规范性。',
    '人工智能辅助治疗技术数据源', NULL, '分子：是指在实施同一术种的人工智能辅助治疗技术时，因术中各种原因导致转为其它手术方式的例数。 分母：是指同期出院患者中同一术种完成人工智能辅助治疗技术的总例数。', '百分比（%）', '病历资料。', NULL,
    '出院日期', '出院科室名称', '是否完成手术中转', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.10.4.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同一术种同期人工智能辅助治疗技术总例数';
SET @denominator_category_code = '5.1.10.4.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成手术中转', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.10.4.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '手术中转率';
SET @category_code = '5.1.10.4';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.10.4"}]'), NULL, NULL, NULL, NULL
);
