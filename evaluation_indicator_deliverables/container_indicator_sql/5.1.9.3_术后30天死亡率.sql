...
-- 创建分子指标
SET @numerator_kpi_name = '心室辅助装置植（介）入术后30天内全因死亡患者数';
SET @numerator_category_code = '5.1.9.3.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '心室辅助装置植（介）入术后30天内死亡患者数（不论何种原因）占同期心室辅助装置植（介）入患者总数的比例。', '', '',
    '心室辅助技术数据源', NULL, '心室辅助装置植（介）入术后30天内死亡患者数（不论何种原因）占同期心室辅助装置植（介）入患者总数的比例。', '百分比（%）', '无', NULL,
    '出院日期', '出院科室名称', '是否完成术后30天死亡', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.9.3.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期心室辅助装置植（介）入患者总数';
SET @denominator_category_code = '5.1.9.3.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成术后30天死亡', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.9.3.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '术后30天死亡率';
SET @category_code = '5.1.9.3';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.9.3"}]'), NULL, NULL, NULL, NULL
);
