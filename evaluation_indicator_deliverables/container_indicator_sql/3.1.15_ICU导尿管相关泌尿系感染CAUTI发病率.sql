...
SET @numerator_kpi_name = 'ICU导尿管相关泌尿系感染发生例数';
SET @numerator_category_code = '3.1.15.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    'ICU导尿管相关泌尿系感染发生例数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU导尿管相关泌尿系感染发生情况',
    '重症医学专病表查询', NULL, '分子：ICU导尿管相关泌尿系感染发生例数。分母：同一统计周期内ICU患者尿导管留置总天数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否ICU导尿管相关泌尿系感染发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同一统计周期内ICU患者尿导管留置总天数';
SET @denominator_category_code = '3.1.15.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同一统计周期内ICU患者尿导管留置总天数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU导尿管相关泌尿系感染发生情况',
    '重症医学专病表查询', NULL, '分子：ICU导尿管相关泌尿系感染发生例数。分母：同一统计周期内ICU患者尿导管留置总天数。', '无', '无', NULL,
    '出院日期', '出院科室名称', 'ICU患者尿导管留置总天数', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = 'ICU导尿管相关泌尿系感染（CAUTI）发病率';
SET @category_code = '3.1.15';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    'CAUTI发生例数占同期ICU患者导尿管留置总天数的比例', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU导尿管相关泌尿系感染发生情况',
    '重症医学专病表查询', NULL, '分子：ICU导尿管相关泌尿系感染发生例数。分母：同一统计周期内ICU患者尿导管留置总天数。', '百分比（%）', '无', NULL,
