-- ===========================================
-- 医疗安全指标 2.3.2 注册脚本（分子+分母+比率）
-- 指标编码：2.3.2 / 2.3.2.1 / 2.3.2.2（以 CSV 为准）
-- 指标名称：（二）手术患者手术后深静脉血栓发生率
-- 创建日期：2026-02
-- ===========================================
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.2"}';

SET @numerator_kpi_name = '手术患者手术后深静脉血栓发生例数';
SET @numerator_category_code = '2.3.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '手术患者手术后深静脉血栓发生例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期管理质量，是医疗安全质量管理的重要指标之一',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：指手术患者手术后深静脉血栓发生的例数。统计住院病案首页的主要诊断或其他诊断以ICD-10："I80.2，I82.8"开头。对应的主要诊断或其他诊断入院病情等于"无"且判定为手术患者的病例数总和。除外病例：①入院时主要诊断为深静脉血栓形成的病例；②在第一次进入手术室之前或入院时发生深静脉血栓形成的病例；分母：指同期手术患者出院人次。',
    '手术患者手术后深静脉血栓发生例数', '无', NULL, '无',
...
SET @denominator_category_code = '2.3.2.2';
CALL sp_kpi_item_create(
    ...
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分母：指同期手术患者出院人次。统计单位以人次数计算，总数为实施各类手术（包含各类介入治疗、腔镜治疗）的总和。不包括妊娠、分娩、围产期、新生儿患者。',
    '同期手术患者出院人次', '无', NULL, '无',
...
SET @kpi_name = '（二）手术患者手术后深静脉血栓发生率';
SET @category_code = '2.3.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, @data_source_param,
    '手术患者手术后深静脉血栓发生率是指手术患者手术后深静脉血栓发生例数占同期手术患者出院人次的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期管理质量，是医疗安全质量管理的重要指标之一',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：指手术患者手术后深静脉血栓发生的例数。统计住院病案首页的主要诊断或其他诊断以ICD-10："I80.2，I82.8"开头。对应的主要诊断或其他诊断入院病情等于"无"且判定为手术患者的病例数总和。除外病例：①入院时主要诊断为深静脉血栓形成的病例；②在第一次进入手术室之前或入院时发生深静脉血栓形成的病例；分母：指同期手术患者出院人次。统计单位以人次数计算，总数为实施各类手术（包含各类介入治疗、腔镜治疗）的总和。不包括妊娠、分娩、围产期、新生儿患者。',
    '（二）手术患者手术后深静脉血栓发生率', '百分比（%）', NULL, '无',
