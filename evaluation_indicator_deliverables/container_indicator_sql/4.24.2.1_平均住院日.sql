-- ===========================================
-- 单病种应上报资源消耗指标注册脚本
-- 指标编码：4.24.2.1（以CSV为准）
-- 指标名称：膝关节置换术平均住院日
-- 创建日期：2026-02
-- 说明：分母 4.24.2.1.2 与 4.24.1.1.2.2（同期手术患者总人数）统计口径一致，采集模式为「源于其他指标」，须先执行 4.24.1.1.2_膝关节功能评估率.sql
-- ===========================================
...
SET @numerator_kpi_name = '膝关节置换术出院患者占用总床日数';
SET @numerator_category_code = '4.24.2.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, @data_source_param,
    '膝关节置换术出院患者占用总床日数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：膝关节置换术出院患者占用总床日数。分母：同期膝关节置换术出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期膝关节置换术出院总人数';
SET @denominator_category_code = '4.24.2.1.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期膝关节置换术出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.24.1.1.2.2（同期手术患者总人数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：膝关节置换术出院患者占用总床日数。分母：同期膝关节置换术出院总人数。', '无', '无', NULL, NULL,
...
SET @kpi_name = '膝关节置换术平均住院日';
SET @category_code = '4.24.2.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'decrease', 'ratio', NULL, @data_source_id, @data_source_param,
    '膝关节置换术出院患者占用总床日数除以同期膝关节置换术出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率，间接反映综合医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：膝关节置换术出院患者占用总床日数。分母：同期膝关节置换术出院总人数。', '天', '无', NULL, NULL,
