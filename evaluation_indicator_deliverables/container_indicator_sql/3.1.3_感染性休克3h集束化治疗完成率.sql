...
SET @numerator_kpi_name = '入ICU诊断为感染性休克并在3h内完成bundle的患者数';
SET @numerator_category_code = '3.1.3.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '入ICU诊断为感染性休克并在3h内完成bundle的患者数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU对感染性休克患者的规范治疗情况',
    '重症医学专病表查询', NULL, '分子：入ICU诊断为感染性休克并在3h内完成bundle的患者数。分母：同一统计周期内入ICU诊断为感染性休克患者总数。除外：住ICU期间后续新发生的感染性休克病例数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否入ICU诊断为感染性休克并在3h内完成bundle', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同一统计周期内入ICU诊断为感染性休克患者总数';
SET @denominator_category_code = '3.1.3.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同一统计周期内入ICU诊断为感染性休克患者总数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU对感染性休克患者的规范治疗情况',
    '重症医学专病表查询', NULL, '分子：入ICU诊断为感染性休克并在3h内完成bundle的患者数。分母：同一统计周期内入ICU诊断为感染性休克患者总数。除外：住ICU期间后续新发生的感染性休克病例数。', '无', '无', NULL,
...
SET @category_code = '3.1.3';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '入ICU诊断为感染性休克并全部完成3h bundle的患者数占同期入ICU诊断为感染性休克患者总数的比例', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU对感染性休克患者的规范治疗情况',
    '重症医学专病表查询', NULL, '分子：入ICU诊断为感染性休克并在3h内完成bundle的患者数。分母：同一统计周期内入ICU诊断为感染性休克患者总数。除外：住ICU期间后续新发生的感染性休克病例数。', '百分比（%）', '无', NULL,
