...
SET @num_name = '住院期间使用隔离措施患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '住院期间使用隔离措施患者数', '无', '反映隔离使用情况', '第四章-住院精神疾病质量控制查询', NULL, 'full', NULL, NULL, '分子：住院期间使用隔离措施患者数。分母：同期出院患者总人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '住院期间使用隔离措施患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.45.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期出院患者总数_4.45.1.2';
-- 分母 4.45.1.2.2 源于 4.45.1.1.2，须先执行 4.45.1.1_精神疾病患者住院期间约束率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期出院患者总人数', '无', '反映隔离使用情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.45.1.1.2（同期出院患者总人数）同源，采集源于该指标', '第四章-住院精神疾病质量控制查询'), NULL, 'full', NULL, NULL, '分子：住院期间使用隔离措施患者数。分母：同期出院患者总人数。（与4.45.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期出院患者总人数', '同期出院患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.45.1.2.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
SET @kpi_name = '住院期间患者隔离率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    '住院期间使用隔离措施患者数占同期出院患者总人数的比例', '无', '反映隔离使用情况', '第四章-住院精神疾病质量控制查询', NULL, 'full', NULL, NULL, '分子：住院期间使用隔离措施患者数。分母：同期出院患者总人数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.45.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
