-- 4.3.1.3 手术并发症发生率（冠状动脉旁路移植术）
-- 分母 4.3.1.3.2 源于 4.3.1.1.2，须先执行 4.3.1.1_首根血管桥乳内动脉使用率.sql
...
SET @numerator_kpi_name = 'CABG手术并发症人数';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, @data_source_param,
    '手术后发生手术并发症的人数', '无', '评价手术治疗质量',
    '第四章-冠状动脉旁路移植术质量控制查询', NULL, 'full', NULL, NULL, '分子：手术后发生手术并发症的人数。分母：同期冠状动脉旁路移植术手术患者总人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '手术后发生手术并发症的人数', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.3.1.3.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

SET @denominator_kpi_name = '同期CABG手术患者总人数_4.3.1.3';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期冠状动脉旁路移植术手术患者总人数', '无', '评价手术治疗质量',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.3.1.1.2（同期行冠状动脉旁路移植术的患者总数）同源，采集源于该指标', '第四章-冠状动脉旁路移植术质量控制查询'), NULL, 'full', NULL, NULL,
    '分子：手术后发生手术并发症的人数。分母：同期冠状动脉旁路移植术手术患者总人数（与4.3.1.1.2一致）。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期行冠状动脉旁路移植术的患者总数', '同期冠状动脉旁路移植术手术患者总人数'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.3.1.3.2"}]'),
...
CALL sp_kpi_item_create(
    ...
    '手术后发生手术并发症的人数占同期冠状动脉旁路移植术手术患者总人数的比例', '无', '评价手术治疗质量',
    '第四章-冠状动脉旁路移植术质量控制查询', NULL, 'full', NULL, NULL, '分子：手术后发生手术并发症的人数。分母：同期冠状动脉旁路移植术手术患者总人数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @numerator_kpi_id, @denominator_kpi_id, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"4.3.1.3"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL
);
