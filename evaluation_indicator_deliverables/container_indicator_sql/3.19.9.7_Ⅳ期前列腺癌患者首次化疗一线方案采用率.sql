...
SET @numerator_kpi_name = 'IV期首次化疗采用一线方案的前列腺癌患者数';
SET @numerator_category_code = '3.19.9.7.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    'IV期首次化疗采用一线方案的前列腺癌患者数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '前列腺癌专病表查询', NULL, '分子：IV期首次化疗采用一线方案的前列腺癌患者数。分母：同期IV期首次化疗的前列腺癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否IV期首次化疗采用一线方案的前列腺癌', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期IV期首次化疗的前列腺癌患者总数';
SET @denominator_category_code = '3.19.9.7.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期IV期首次化疗的前列腺癌患者总数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '前列腺癌专病表查询', NULL, '分子：IV期首次化疗采用一线方案的前列腺癌患者数。分母：同期IV期首次化疗的前列腺癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期IV期首次化疗的前列腺癌患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = 'Ⅳ期前列腺癌患者首次化疗一线方案采用率';
SET @category_code = '3.19.9.7';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    'Ⅳ期前列腺癌患者首次化疗一线方案采用率的定义和计算方法', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '前列腺癌专病表查询', NULL, '分子：IV期首次化疗采用一线方案的前列腺癌患者数。分母：同期IV期首次化疗的前列腺癌患者总数。', '百分比（%）', '无', NULL,
