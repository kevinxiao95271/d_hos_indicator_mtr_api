...
SET @numerator_category_code = '3.1.10.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    'ICU气管插管计划拔管后48h内再插管例数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU气管插管拔管管理质量',
    '重症医学专病表查询', NULL, '分子：ICU气管插管计划拔管后48h内再插管例数。分母：同一统计周期内ICU患者气管插管拔管总数，不包括非计划气管插管拔管后再插管例数。', '无', '无', NULL,
...
SET @denominator_kpi_name = '同一统计周期内ICU患者气管插管拔管总数，不包括非计划气管插管拔管后再插管例数';
SET @denominator_category_code = '3.1.10.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同一统计周期内ICU患者气管插管拔管总数，不包括非计划气管插管拔管后再插管例数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU气管插管拔管管理质量',
    '重症医学专病表查询', NULL, '分子：ICU气管插管计划拔管后48h内再插管例数。分母：同一统计周期内ICU患者气管插管拔管总数，不包括非计划气管插管拔管后再插管例数。', '无', '无', NULL,
...
SET @kpi_name = 'ICU气管插管拔管后48h内再插管率';
SET @category_code = '3.1.10';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '气管插管计划拔管后48h内再插管例数占同期ICU患者气管插管拔管总例数的比例', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU气管插管拔管管理质量',
    '重症医学专病表查询', NULL, '分子：ICU气管插管计划拔管后48h内再插管例数。分母：同一统计周期内ICU患者气管插管拔管总数，不包括非计划气管插管拔管后再插管例数。', '百分比（%）', '无', NULL,
