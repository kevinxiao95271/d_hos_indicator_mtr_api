...
-- 创建分子指标
SET @numerator_kpi_name = '活胰岛数';
SET @numerator_category_code = '5.1.2.3.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '采用活细胞染色技术进行胰岛计数。胰岛活率是指活胰岛数占胰岛总数的比例。', '《同种胰岛移植技术临床应用管理规范（2022 版）》《同种胰岛移植技术临床应用质量控制指标（2022 版）》。', '用于评价获取胰岛中活细胞的比例，体现胰岛提取技术水平的指标。',
    '同种胰岛移植技术数据源', NULL, '无', '百分比（%）', '医院胰岛制备及检测过程记录和检测报告。', NULL,
    '出院日期', '出院科室名称', '是否完成胰岛活率', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.2.3.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '胰岛总数';
SET @denominator_category_code = '5.1.2.3.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成胰岛活率', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.2.3.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '胰岛活率';
SET @category_code = '5.1.2.3';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.2.3"}]'), NULL, NULL, NULL, NULL
);
