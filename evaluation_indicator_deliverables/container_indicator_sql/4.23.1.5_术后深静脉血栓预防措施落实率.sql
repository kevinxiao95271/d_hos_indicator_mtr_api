-- 4.23.1.5 术后深静脉血栓预防措施落实率（髋关节置换术）
-- 分母 4.23.1.5.2 与 4.23.1.1.1.2 统计口径一致，采集源于该指标（须先执行 4.23.1.1.1）
...
SET @num_name = '髋关节置换术进行深静脉血栓预防患者数_4.23.1.5';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '进行深静脉血栓预防的患者数', '无', '反映患者VTE预防情况', '第四章-髋关节置换术质量控制查询', NULL, 'full', NULL, NULL, '分子：单位时间进行深静脉血栓预防的患者数。分母：同期手术患者总例数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '进行深静脉血栓预防的患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.23.1.5.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.23.1.5.2';
SET @den_name = '同期髋关节置换术手术患者总数_4.23.1.5';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期手术患者总数', '无', '反映患者VTE预防情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.23.1.1.1.2（同期手术患者总人数）同源，采集源于该指标', '第四章-髋关节置换术质量控制查询'), NULL, 'full', NULL, NULL, '分子：单位时间进行深静脉血栓预防的患者数。分母：同期手术患者总例数。（与4.23.1.1.1.2一致）', '无', '无', NULL, NULL,
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '进行深静脉血栓预防的患者数占同期手术患者总例数的比例', '无', '反映患者VTE预防情况', '第四章-髋关节置换术质量控制查询', NULL, 'full', NULL, NULL, '分子：单位时间进行深静脉血栓预防的患者数。分母：同期手术患者总例数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.23.1.5"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
