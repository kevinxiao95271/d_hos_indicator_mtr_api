-- 4.16.1.4 病死率（惊厥性癫痫持续状态）
-- 分母 4.16.1.4.2 与 4.16.1.1.2 统计口径一致，采集源于该指标（须先执行 4.16.1.1）
...
SET @num_name = 'CSE死亡人数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '某病种的死亡人数', '无', '评价惊厥性癫痫持续状态患者住院死亡情况', '第四章-惊厥性癫痫持续状态质量控制查询', NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '某病种的死亡人数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.16.1.4.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.16.1.4.2';
SET @source_kpi_id = (
...
SET @den_name = '同期CSE出院总人数_4.16.1.4';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期某病种出院总人数', '无', '评价惊厥性癫痫持续状态患者住院死亡情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.16.1.1.2（同期住院惊厥性癫痫持续状态患者总数）同源，采集源于该指标', '第四章-惊厥性癫痫持续状态质量控制查询'), NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期住院惊厥性癫痫持续状态患者总数', '同期某病种出院总人数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    '某病种的死亡人数占同期某病种出院总人数的比例', '无', '评价惊厥性癫痫持续状态患者住院死亡情况', '第四章-惊厥性癫痫持续状态质量控制查询', NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.16.1.4"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
