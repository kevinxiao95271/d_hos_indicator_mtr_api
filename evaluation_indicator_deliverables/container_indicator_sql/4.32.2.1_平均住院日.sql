-- ===========================================
-- 单病种应上报资源消耗指标注册脚本
-- 指标编码：4.32.2.1（以CSV为准）
-- 指标名称：胃癌（手术治疗）平均住院日
-- 创建日期：2026-02
-- 说明：分母 4.32.2.1.2 与 4.32.1.1.2（同期胃癌手术患者总例数）同源，须先执行 4.32.1.1 注册脚本。
-- ===========================================
...
SET @category_id = (
    ...
    WHERE CATEGORY_NAME LIKE '%胃癌%' AND INVALID = 0
    LIMIT 1
);

SET @create_user = 'system';
SET @data_source_param = '{"p_date_type":"out","p_disease_code":"GASTRIC_CA"}';

SET @numerator_kpi_name = '胃癌（手术治疗）出院患者占用总床日数';
SET @numerator_category_code = '4.32.2.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @data_source_id, @data_source_param,
    '胃癌（手术治疗）出院患者占用总床日数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：胃癌（手术治疗）出院患者占用总床日数。分母：同期胃癌（手术治疗）出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期胃癌（手术治疗）出院总人数';
SET @denominator_category_code = '4.32.2.1.2';
SET @source_kpi_id = (
    SELECT k.ID FROM KPI_ITEM k
    INNER JOIN KPI_ITEM_CATEGORY_REL r ON r.KPI_ID = k.ID AND r.CATEGORY_CODE = '4.32.1.1.2'
...
CALL sp_kpi_item_create(
    ...
    '同期胃癌（手术治疗）出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.32.1.1.2（同期胃癌手术患者总例数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：胃癌（手术治疗）出院患者占用总床日数。分母：同期胃癌（手术治疗）出院总人数。（与4.32.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期胃癌手术患者总例数', '计数'), NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @kpi_name = '胃癌（手术治疗）平均住院日';
SET @category_code = '4.32.2.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'decrease', 'ratio', NULL, @data_source_id, @data_source_param,
    '胃癌（手术治疗）出院患者占用总床日数除以同期胃癌（手术治疗）出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的时间消耗情况，反映医疗效率，间接反映综合医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：胃癌（手术治疗）出院患者占用总床日数。分母：同期胃癌（手术治疗）出院总人数。', '天', '无', NULL, NULL,
