-- 4.43.1.3 预防性抗菌药物在术后48小时内停药率（围手术期预防感染）
-- 分母 4.43.1.3.2 与 4.43.1.1.2 统计口径一致，采集源于该指标（须先执行 4.43.1.1）
...
SET @num_name = '预防性抗菌药物术后48小时内停药患者人数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '预防性抗菌药物在术后48小时内停药的患者人数', '无', '反映预防用抗菌药物停药规范性', '第四章-围手术期预防感染质量控制查询', NULL, 'full', NULL, NULL, '分子：预防性抗菌药物在术后48小时内停药的患者人数。分母：同期所有预防使用抗菌药物的患者人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '预防性抗菌药物在术后48小时内停药的患者人数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.43.1.3.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.43.1.3.2';
SET @source_kpi_id = (
...
SET @den_name = '同期预防使用抗菌药物患者人数_4.43.3';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期所有预防使用抗菌药物的患者人数', '无', '反映预防用抗菌药物停药规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.43.1.1.2（同期所有预防使用抗菌药物的患者总数）同源，采集源于该指标', '第四章-围手术期预防感染质量控制查询'), NULL, 'full', NULL, NULL, '分子：预防性抗菌药物在术后48小时内停药的患者人数。分母：同期所有预防使用抗菌药物的患者人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期所有预防使用抗菌药物的患者总数', '同期所有预防使用抗菌药物的患者人数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
SET @kpi_name = '预防性抗菌药物在术后48小时内停药率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '预防性抗菌药物在术后48小时内停药的患者人数占同期所有预防使用抗菌药物患者人数的比例', '无', '反映预防用抗菌药物停药规范性', '第四章-围手术期预防感染质量控制查询', NULL, 'full', NULL, NULL, '分子：预防性抗菌药物在术后48小时内停药的患者人数。分母：同期所有预防使用抗菌药物的患者人数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.43.1.3"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
