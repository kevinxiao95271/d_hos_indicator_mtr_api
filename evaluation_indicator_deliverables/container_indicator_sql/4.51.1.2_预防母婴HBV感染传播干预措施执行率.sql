-- 4.51.1.2 预防母婴HBV感染传播干预措施执行率（HBV感染分娩母婴阻断）
...
SET @num_name = '实施预防母婴HBV传播干预新生儿数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '实施预防母婴HBV感染传播干预的新生儿数', '无', '反映母婴阻断干预落实', '第四章-HBV感染分娩母婴阻断质量控制查询', NULL, 'full', NULL, NULL, '分子：实施预防母婴HBV感染传播干预的新生儿数。分母：同期HBV感染孕产妇总人数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '实施预防母婴HBV感染传播干预的新生儿数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.51.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期HBV母亲分娩新生儿总数_4.51.1.2';
-- 分母 4.51.1.2.2 源于 4.51.1.1.2，须先执行 4.51.1.1_HBV感染分娩母婴传播风险评估率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期HBV感染孕产妇总人数', '无', '反映母婴阻断干预落实', IF(@source_kpi_id IS NOT NULL, '与指标编码4.51.1.1.2（同期HBV感染孕产妇总人数）同源，采集源于该指标', '第四章-HBV感染分娩母婴阻断质量控制查询'), NULL, 'full', NULL, NULL, '分子：实施预防母婴HBV感染传播干预的新生儿数。分母：同期HBV感染孕产妇总人数。（与4.51.1.1.2一致）', '无', '无', NULL, NULL,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.51.1.2.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
SET @kpi_name = '预防母婴HBV感染传播干预措施执行率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '实施预防母婴HBV感染传播干预的新生儿数占同期HBV感染孕产妇总人数的比例', '无', '反映母婴阻断干预落实', '第四章-HBV感染分娩母婴阻断质量控制查询', NULL, 'full', NULL, NULL, '分子：实施预防母婴HBV感染传播干预的新生儿数。分母：同期HBV感染孕产妇总人数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.51.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
