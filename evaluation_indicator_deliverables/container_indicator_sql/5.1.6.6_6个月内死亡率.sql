...
-- 创建分子指标
SET @numerator_kpi_name = '6个月内死亡的患者数';
SET @numerator_category_code = '5.1.6.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '6个月内死亡是指患者从第一次接受质子或重离子放射治疗起，6个月之内死亡。6个月内死亡率是指实施质子或重离子放射治疗的患者，6个月内死亡的患者数占同期质子或重离子放射治疗患者总数的比例。', '', '',
    '质子和重离子加速器放射治疗技术数据源', NULL, '6个月内死亡是指患者从第一次接受质子或重离子放射治疗起，6个月之内死亡。6个月内死亡率是指实施质子或重离子放射治疗的患者，6个月内死亡的患者数占同期质子或重离子放射治疗患者总数的比例。', '百分比（%）', '无', NULL,
    '出院日期', '出院科室名称', '是否完成6个月内死亡', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.6.6.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期质子或重离子放射治疗患者总数';
SET @denominator_category_code = '5.1.6.6.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成6个月内死亡', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.6.6.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '6个月内死亡率';
SET @category_code = '5.1.6.6';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.6.6"}]'), NULL, NULL, NULL, NULL
);
