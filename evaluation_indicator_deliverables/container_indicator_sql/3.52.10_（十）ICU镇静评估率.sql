-- ===========================================
-- 0352 专病表指标注册脚本
-- 指标编码：3.52.10（十）ICU镇静评估率
-- 数据源：重症医学专病表查询（需扩展 镇静评估 值字段）
-- ===========================================
...
SET @numerator_kpi_name = '进行镇静评估的ICU患者人数';
SET @numerator_category_code = '3.52.10.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', NULL, @data_source_id, '{"p_date_type":"out"}',
    '进行镇静评估的ICU患者人数', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映对重症患者镇静评估情况',
    '重症医学专病表查询', NULL, 'full', '分子：进行镇静评估的ICU患者人数。分母：同期ICU患者总人数。', '无', '无', NULL, '无', '无', '无',
    '出院日期', '出院科室名称', '是否进行镇静评估', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期ICU患者总人数_镇静';
SET @denominator_category_code = '3.52.10.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', NULL, @data_source_id, '{"p_date_type":"out"}',
    '同期ICU患者总人数', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映对重症患者镇静评估情况',
    '重症医学专病表查询', NULL, 'full', '分子：进行镇静评估的ICU患者人数。分母：同期ICU患者总人数。', '无', '无', NULL, '无', '无', '无',
...
SET @kpi_name = '（十）ICU镇静评估率';
SET @category_code = '3.52.10';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', NULL, @data_source_id, '{"p_date_type":"out"}',
    '进行镇静评估的ICU患者人数占同期ICU患者总人数的比例', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映对重症患者镇静评估情况',
    '重症医学专病表查询', NULL, 'full', '镇静评估指采用RASS、SAS等工具进行镇静评估。', NULL, NULL, '百分比（%）', '无', '无', '无',
