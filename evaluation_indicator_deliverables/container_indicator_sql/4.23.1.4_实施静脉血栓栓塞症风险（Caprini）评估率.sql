-- 4.23.1.4 实施静脉血栓栓塞症风险（Caprini）评估率（髋关节置换术）
-- 分母 4.23.1.4.2 与 4.23.1.1.1.2 统计口径一致，采集源于该指标（须先执行 4.23.1.1.1）
...
SET @num_name = '髋关节置换术实施Caprini评估患者数_4.23.1.4.1';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.23.1.4.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.23.1.4.2';
SET @den_name = '同期髋关节置换术手术患者总数_4.23.1.4.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期手术患者总数', '无', '反映患者VTE预防情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.23.1.1.1.2（同期手术患者总人数）同源，采集源于该指标', '第四章-髋关节置换术质量控制查询'), NULL, 'full', NULL, NULL, '分子：实施静脉血栓栓塞症风险（Caprini）评估的患者数。分母：同期手术患者总例数。（与4.23.1.1.1.2一致）', '无', '无', NULL, NULL,
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.23.1.4"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
