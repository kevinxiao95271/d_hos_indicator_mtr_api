-- 4.21.1.4 抗菌药物使用合理率（哮喘成人，急性发作，住院）
-- 分母 4.21.1.4.2 与 4.21.1.1.2 统计口径一致，采集源于该指标（须先执行 4.21.1.1）
...
SET @num_name = '哮喘成人合理应用抗菌药物患者数_4.21.1.4';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '合理应用抗菌药物的患者例数', '无', '反映抗菌药物合理使用情况', '第四章-哮喘（成人，急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：按诊疗指南合理应用抗菌药物的患者数。分母：同期支气管哮喘急性发作出院患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '合理应用抗菌药物的患者例数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.21.1.4.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.21.1.4.2';
SET @source_kpi_id = (
...
SET @den_name = '同期本病种出院患者总例数_4.21.1.4';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期本病种出院患者总例数', '无', '反映抗菌药物合理使用情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.21.1.1.2（同期支气管哮喘急性发作出院患者总例数）同源，采集源于该指标', '第四章-哮喘（成人，急性发作，住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：按诊疗指南合理应用抗菌药物的患者数。分母：同期支气管哮喘急性发作出院患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期支气管哮喘急性发作出院患者总数', '同期支气管哮喘急性发作出院患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
SET @kpi_name = '抗菌药物使用合理率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '按诊疗指南合理应用抗菌药物的患者数占同期支气管哮喘急性发作出院患者总数的比例', '无', '反映抗菌药物合理使用情况', '第四章-哮喘（成人，急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：按诊疗指南合理应用抗菌药物的患者数。分母：同期支气管哮喘急性发作出院患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.21.1.4"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
