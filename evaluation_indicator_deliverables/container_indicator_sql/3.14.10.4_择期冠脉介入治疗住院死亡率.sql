...
-- 创建分子指标（3.14.10.4.1）
SET @numerator_kpi_name = '择期冠脉介入治疗住院期间死亡的患者数';
SET @numerator_category_code = '3.14.10.4.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '择期冠脉介入治疗住院期间死亡的患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '冠心病介入治疗专病表查询', NULL, '分子：择期冠脉介入治疗住院期间死亡的患者数。择期冠脉介入治疗是指除ST段抬高型心肌梗死（STEMI）患者接受的直接经皮冠状动脉介入治疗（PCI）以及非ST段抬高型急性冠脉综合征（NSTE ACS）患者接受的急诊经皮冠状动脉介入治疗（PCI）以外的介入治疗。分母：同期接受择期冠脉介入治疗的患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否住院期间死亡_择期', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建分母指标（3.14.10.4.2）
SET @denominator_kpi_name = '同期接受择期冠脉介入治疗的患者总数';
SET @denominator_category_code = '3.14.10.4.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期接受择期冠脉介入治疗的患者总数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '冠心病介入治疗专病表查询', NULL, '分子：择期冠脉介入治疗住院期间死亡的患者数。择期冠脉介入治疗是指除ST段抬高型心肌梗死（STEMI）患者接受的直接经皮冠状动脉介入治疗（PCI）以及非ST段抬高型急性冠脉综合征（NSTE ACS）患者接受的急诊经皮冠状动脉介入治疗（PCI）以外的介入治疗。分母：同期接受择期冠脉介入治疗的患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期接受择期冠脉介入治疗的患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
-- 创建比率指标（3.14.10.4）
SET @kpi_name = '择期冠脉介入治疗住院死亡率';
SET @category_code = '3.14.10.4';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '择期冠脉介入治疗住院期间死亡的患者数，占同期接受择期冠脉介入治疗的患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价手术诊疗质量情况',
    '冠心病介入治疗专病表查询', NULL, '分子：择期冠脉介入治疗住院期间死亡的患者数。择期冠脉介入治疗是指除ST段抬高型心肌梗死（STEMI）患者接受的直接经皮冠状动脉介入治疗（PCI）以及非ST段抬高型急性冠脉综合征（NSTE ACS）患者接受的急诊经皮冠状动脉介入治疗（PCI）以外的介入治疗。分母：同期接受择期冠脉介入治疗的患者总数。', '百分比（%）', '无', NULL,
