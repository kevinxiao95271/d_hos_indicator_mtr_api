...
SET @numerator_kpi_name = '严重产后出血输血治疗人数';
SET @numerator_category_code = '3.8.6.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '严重产后出血输血治疗人数', '《产科专业医疗质量控制指标（2021年版）》。', '评价严重产后出血患者输血治疗情况',
    '产科专病表查询', NULL, '分子：严重产后出血输血治疗人数。分母：同期严重产后出血患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否严重产后出血输血治疗', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期严重产后出血患者总数';
SET @denominator_category_code = '3.8.6.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期严重产后出血患者总数', '《产科专业医疗质量控制指标（2021年版）》。', '评价严重产后出血患者输血治疗情况',
    '产科专病表查询', NULL, '分子：严重产后出血输血治疗人数。分母：同期严重产后出血患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否严重产后出血', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = '严重产后出血患者输血率';
SET @category_code = '3.8.6';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，发生严重产后出血（分娩24小时内出血量≥1000ml）实施输血治疗（含自体输血）人数占同期发生严重产后出血患者总数的比例', '《产科专业医疗质量控制指标（2021年版）》。', '评价严重产后出血患者输血治疗情况',
    '产科专病表查询', NULL, '分子：严重产后出血输血治疗人数。分母：同期严重产后出血患者总数。', '百分比（%）', '无', NULL,
