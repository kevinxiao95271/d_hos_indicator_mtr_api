-- 4.20.1.5 危重患者机械通气治疗率（慢性阻塞性肺疾病急性发作，住院）
-- 分母 4.20.1.5.2 与 4.20.1.1.2 统计口径一致，采集源于该指标（须先执行 4.20.1.1）
...
SET @num_name = 'COPD危重患者机械通气治疗患者总数_4.20.1.5';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '住院期间使用机械通气的危重患者总数', '无', '反映慢性阻塞性肺疾病急性加重危重症患者治疗的规范性', '第四章-慢性阻塞性肺疾病（急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：住院期间应用无创正压通气、有创机械通气的患者总数。分母：同期慢性阻塞性肺疾病急性加重危重患者的出院总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '住院期间使用机械通气的危重患者总数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.20.1.5.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.20.1.5.2';
SET @source_kpi_id = (
...
SET @den_name = '同期COPD急性加重危重患者出院总数_4.20.1.5';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期慢性阻塞性肺疾病急性加重危重患者的出院总数', '无', '反映慢性阻塞性肺疾病急性加重危重症患者治疗的规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.20.1.1.2（同期慢性阻塞性肺疾病急性加重出院患者总数）同源，采集源于该指标', '第四章-慢性阻塞性肺疾病（急性发作，住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：住院期间应用无创正压通气、有创机械通气的患者总数。分母：同期慢性阻塞性肺疾病急性加重危重患者的出院总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期慢性阻塞性肺疾病急性加重出院患者总数', '同期慢性阻塞性肺疾病急性加重危重患者的出院总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
SET @kpi_name = '危重患者机械通气治疗率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '住院期间应用无创正压通气、有创机械通气的患者总数占同期慢性阻塞性肺疾病急性加重危重患者出院总数的比例', '无', '反映慢性阻塞性肺疾病急性加重危重症患者治疗的规范性', '第四章-慢性阻塞性肺疾病（急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：住院期间应用无创正压通气、有创机械通气的患者总数。分母：同期慢性阻塞性肺疾病急性加重危重患者的出院总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.20.1.5"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
