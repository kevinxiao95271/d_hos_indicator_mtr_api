...
-- 创建分子指标
SET @numerator_kpi_name = '心室辅助装置植（介）入后一定时间内随访尚存活的患者数';
SET @numerator_category_code = '5.1.9.7.1';
CALL sp_kpi_item_create(
    ...
    '心室辅助技术数据源', NULL, '分子：是指患者在完成心室辅助装置植入术后，按照 1、 3 、5 年完成随访尚存活的患者数量。 分母：是指同期完成心室辅助装置应用的患者总数。', '百分比（%）', '随访记录。', NULL,
    '出院日期', '出院科室名称', '是否完成患者术后生存', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.9.7.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期心室辅助装置植（介）入患者总数';
SET @denominator_category_code = '5.1.9.7.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成患者术后生存', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.9.7.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '患者术后生存率';
SET @category_code = '5.1.9.7';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.9.7"}]'), NULL, NULL, NULL, NULL
);
