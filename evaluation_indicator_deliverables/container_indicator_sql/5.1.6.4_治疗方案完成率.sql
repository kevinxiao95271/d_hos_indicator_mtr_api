...
-- 创建分子指标
SET @numerator_kpi_name = '完成既定治疗方案的患者数';
SET @numerator_category_code = '5.1.6.4.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '实施质子或重离子放射治疗的患者，完成既定治疗方案的患者数占同期质子或重离子放射治疗患者总数的比例。', '《质子和重离子加速器放射治疗技术临床应用管理规范（2022 版）》《质子和重离子加速器放射治疗技术临床应用质量控制指标 （2022 版）》。', '反映医院质子或重离子放射治疗的规范性。',
    '质子和重离子加速器放射治疗技术数据源', NULL, '分子：是指为患者制定质子或重离子放射治疗方案，并落实完成的患者数量。 分母：是指同期接受质子或者重离子放射治疗的患者总数。', '百分比（%）', '病历资料、病例信息数据库。', NULL,
    '出院日期', '出院科室名称', '是否完成治疗方案完成', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.6.4.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期质子或重离子放射治疗患者总数';
SET @denominator_category_code = '5.1.6.4.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成治疗方案完成', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.6.4.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '治疗方案完成率';
SET @category_code = '5.1.6.4';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.6.4"}]'), NULL, NULL, NULL, NULL
);
