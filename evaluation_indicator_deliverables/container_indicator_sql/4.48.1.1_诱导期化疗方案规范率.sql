-- 4.48.1.1 诱导期化疗方案规范率（儿童急性淋巴细胞白血病）
-- 说明：分母编码 4.48.1.1.2（同期该病种住院患儿总数）为 4.48.1 及资源消耗 4.48.2 统一源分母；
--       4.48.1.2.2、4.48.1.3.2、4.48.2.1.2、4.48.2.2.2、4.48.2.3.2 等与本章要求一致的指标分母，注册脚本中采集源于本指标。
...
SET @num_name = '诱导期化疗规范制定方案患儿例数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '诱导期化疗规范制定化疗方案的患儿例数', '无', '反映诱导期化疗方案规范性', '第四章-儿童急性淋巴细胞白血病（初始诱导化疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：诱导期化疗规范制定化疗方案的患儿例数。分母：同期该病种住院患儿总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '诱导期化疗规范制定化疗方案的患儿例数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.48.1.1.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期该病种住院患儿总数_4.48.1.1';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '同期该病种住院患儿总数', '无', '反映诱导期化疗方案规范性', '第四章-儿童急性淋巴细胞白血病（初始诱导化疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：诱导期化疗规范制定化疗方案的患儿例数。分母：同期该病种住院患儿总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '同期该病种住院患儿总数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.48.1.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '诱导期化疗规范制定化疗方案的患儿例数占同期该病种住院患儿总数的比例', '无', '反映诱导期化疗方案规范性', '第四章-儿童急性淋巴细胞白血病（初始诱导化疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：诱导期化疗规范制定化疗方案的患儿例数。分母：同期该病种住院患儿总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.48.1.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
