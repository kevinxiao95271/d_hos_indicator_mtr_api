...
-- 创建分子指标（3.14.4.1.1）
SET @numerator_kpi_name = '住院期间接受动态血压监测的高血压患者数';
SET @numerator_category_code = '3.14.4.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '住院期间接受动态血压监测的高血压患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院对高血压患者规范评估情况',
    '高血压专病表查询', NULL, '分子：住院期间接受动态血压监测的高血压患者数。动态血压监测是指通过自动血压测量仪器监测血压水平。分母：同期高血压住院患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否住院期间接受动态血压监测', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.4.1.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.4.1）
SET @kpi_name = '动态血压监测率';
SET @category_code = '3.14.4.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '住院期间接受动态血压监测的高血压患者数，占同期高血压住院患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院对高血压患者规范评估情况',
    '高血压专病表查询', NULL, '分子：住院期间接受动态血压监测的高血压患者数。动态血压监测是指通过自动血压测量仪器监测血压水平。分母：同期高血压住院患者总数。', '百分比（%）', '无', NULL,
