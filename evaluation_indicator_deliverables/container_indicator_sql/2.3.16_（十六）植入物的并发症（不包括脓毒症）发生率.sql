-- 医疗安全指标 2.3.16 注册脚本（分子+分母+比率）
-- 指标编码：2.3.16 / 2.3.16.1 / 2.3.16.2
-- 指标名称：（十六）植入物的并发症（不包括脓毒症）发生率
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.16"}';

SET @numerator_kpi_name = '植入物的并发症发生例数';
SET @numerator_category_code = '2.3.16.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '植入物的并发症发生例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：植入物的并发症发生例数；分母：同期行植入物手术患者出院人次。',
    '植入物的并发症发生例数', '无', NULL, '无',
...
SET @denominator_kpi_name = '同期行植入物手术患者出院人次';
SET @denominator_category_code = '2.3.16.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '同期行植入物手术患者出院人次', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分母：同期行植入物手术患者出院人次。',
    '同期行植入物手术患者出院人次', '无', NULL, '无',
...
SET @kpi_name = '（十六）植入物的并发症（不包括脓毒症）发生率';
SET @category_code = '2.3.16';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, @data_source_param,
    '植入物的并发症（不包括脓毒症）发生率是指手术患者手术后植入物的并发症发生例数占同期植入物手术患者出院人次的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：植入物的并发症发生例数；分母：同期行植入物手术患者出院人次。',
    '（十六）植入物的并发症（不包括脓毒症）发生率', '百分比（%）', NULL, '无',
