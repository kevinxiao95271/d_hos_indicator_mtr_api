-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.9.4.2（与CSV文件中的编码完全一致）
-- 指标名称：颈动脉支架置入术患者术前颈动脉无创影像评估率
-- 创建日期：2026-02-05
-- ===========================================
...
-- 创建分子指标（3.9.4.2.1）
SET @numerator_kpi_name = '术前行颈动脉无创影像评估的颈动脉支架置入术患者数';
SET @numerator_category_code = '3.9.4.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '术前行颈动脉无创影像评估的颈动脉支架置入术患者数', '《神经系统疾病医疗质量控制指标（2020 年版）》。 2.颈动脉支架置入术患者术前颈动脉无创影像评估率', '反映医疗机构对颈动脉支架置入术患者术前规范化评估的现状。',
    '颈动脉支架置入术专病表查询', NULL, '颈动脉无创影像评估包含：颈部血管彩超、颈动脉CTA、颈动脉CE-MRA、颈动脉MRA。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否术前行颈动脉无创影像评估的颈动脉支架置入术', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.9.4.2.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.9.4.2）
SET @kpi_name = '颈动脉支架置入术患者术前颈动脉无创影像评估率';
SET @category_code = '3.9.4.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，术前行颈动脉无创影像评估的颈动脉支架置入术患者数占颈动脉支架置入术患者总数的比例。', '《神经系统疾病医疗质量控制指标（2020 年版）》。 2.颈动脉支架置入术患者术前颈动脉无创影像评估率', '反映医疗机构对颈动脉支架置入术患者术前规范化评估的现状。',
    '颈动脉支架置入术专病表查询', NULL, '颈动脉无创影像评估包含：颈部血管彩超、颈动脉CTA、颈动脉CE-MRA、颈动脉MRA。', '百分比（%）', '无', NULL,
