-- 4.35.1.3 肾脏组织活检病理检查率（糖尿病肾病）
-- 分母 4.35.1.3.2 与 4.35.1.1.2 统计口径一致，采集源于该指标（须先执行 4.35.1.1）
...
SET @num_name = '糖尿病肾病完成肾脏组织活检病理检查例数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '完成肾脏组织活检病理检查例数', '无', '反映病理诊断规范性', '第四章-糖尿病肾病质量控制查询', NULL, 'full', NULL, NULL, '分子：完成肾脏组织活检病理检查例数。分母：同期该病种患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '完成肾脏组织活检病理检查例数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.35.1.3.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.35.1.3.2';
SET @source_kpi_id = (
...
SET @den_name = '同期糖尿病肾病病种患者总数_4.35.1.3';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期该病种患者总数', '无', '反映病理诊断规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.35.1.1.2（同期出院患者总人数）同源，采集源于该指标', '第四章-糖尿病肾病质量控制查询'), NULL, 'full', NULL, NULL, '分子：完成肾脏组织活检病理检查例数。分母：同期该病种患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期出院患者总人数', '同期该病种患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
SET @kpi_name = '肾脏组织活检病理检查率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '完成肾脏组织活检病理检查例数占同期该病种患者总数的比例', '无', '反映病理诊断规范性', '第四章-糖尿病肾病质量控制查询', NULL, 'full', NULL, NULL, '分子：完成肾脏组织活检病理检查例数。分母：同期该病种患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.35.1.3"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
