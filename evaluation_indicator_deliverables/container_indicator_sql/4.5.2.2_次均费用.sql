...
-- 指标名称：主动脉瓣置换术次均费用
-- 创建日期：2026-02
-- 说明：分母 4.5.2.2.2 与 4.5.1.1.2（同期手术患者的总例数）统计口径一致，采集模式为「源于其他指标」，须先执行 4.5.1.1_术前评估完成率.sql
-- ===========================================
...
SET @numerator_kpi_name = '主动脉瓣置换术患者住院费用之和';
SET @numerator_category_code = '4.5.2.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'value', NULL, @data_source_id, @data_source_param,
    '所有主动脉瓣置换术患者住院费用之和', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：所有主动脉瓣置换术患者住院费用之和。分母：同期主动脉瓣置换术出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期主动脉瓣置换术出院总人数_次均费用';
SET @denominator_category_code = '4.5.2.2.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期主动脉瓣置换术出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.5.1.1.2（同期手术患者的总例数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：所有主动脉瓣置换术患者住院费用之和。分母：同期主动脉瓣置换术出院总人数（与4.5.1.1.2一致）。', '无', '无', NULL, NULL,
...
SET @kpi_name = '主动脉瓣置换术次均费用';
SET @category_code = '4.5.2.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'ratio', NULL, @data_source_id, @data_source_param,
    '所有主动脉瓣置换术患者住院费用之和除以同期主动脉瓣置换术出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制，间接反映医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：所有主动脉瓣置换术患者住院费用之和。分母：同期主动脉瓣置换术出院总人数。', '元', '无', NULL, NULL,
