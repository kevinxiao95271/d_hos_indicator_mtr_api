-- ===========================================
-- 0352 专病表指标注册脚本
-- 指标编码：3.52.5.3 感染性休克6h集束化治疗（bundle）完成率
-- 数据源：重症医学专病表查询
-- ===========================================
...
SET @numerator_kpi_name = '入ICU诊断为感染性休克并全部完成6h bundle的患者数';
SET @numerator_category_code = '3.52.5.3.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', NULL, @data_source_id, '{"p_date_type":"out"}',
    '入ICU诊断为感染性休克并全部完成6h bundle的患者数', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映感染性休克的治疗规范性及诊疗能力',
    '重症医学专病表查询', NULL, 'full', '分子：入ICU诊断为感染性休克并全部完成6h bundle的患者数。分母：同期入ICU诊断为感染性休克患者总数。', '无', '无', NULL, '无', '无', '无',
...
SET @denominator_kpi_name = '同期入ICU诊断为感染性休克患者总数';
SET @denominator_category_code = '3.52.5.3.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', NULL, @data_source_id, '{"p_date_type":"out"}',
    '同期入ICU诊断为感染性休克患者总数', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映感染性休克的治疗规范性及诊疗能力',
    '重症医学专病表查询', NULL, 'full', '分子：入ICU诊断为感染性休克并全部完成6h bundle的患者数。分母：同期入ICU诊断为感染性休克患者总数。', '无', '无', NULL, '无', '无', '无',
...
SET @category_code = '3.52.5.3';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @data_source_id, '{"p_date_type":"out"}',
    '入ICU诊断为感染性休克并完成bundle的患者人数占同期入ICU诊断为感染性休克的患者总人数的比例（6小时）', '《重症医学专业医疗质量控制指标（2024年版）》。', '反映感染性休克的治疗规范性及诊疗能力',
