...
SET @numerator_category_code = '3.1.12.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '转出ICU后48h内重返ICU的患者数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU转出患者管理质量',
    '重症医学专病表查询', NULL, '分子：转出ICU后48h内重返ICU的患者数。分母：同一统计周期内转出ICU患者总数。', '无', '无', NULL,
...
SET @denominator_kpi_name = '同一统计周期内转出ICU患者总数';
SET @denominator_category_code = '3.1.12.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同一统计周期内转出ICU患者总数', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU转出患者管理质量',
    '重症医学专病表查询', NULL, '分子：转出ICU后48h内重返ICU的患者数。分母：同一统计周期内转出ICU患者总数。', '无', '无', NULL,
...
SET @kpi_name = '转出ICU后48h内重返率';
SET @category_code = '3.1.12';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '转出ICU后48h内重返ICU的患者数占同期转出ICU患者总数的比例', '《重症医学专业医疗质量控制指标（2021年版）》。', '评价ICU转出患者管理质量',
    '重症医学专病表查询', NULL, '分子：转出ICU后48h内重返ICU的患者数。分母：同一统计周期内转出ICU患者总数。', '百分比（%）', '无', NULL,
