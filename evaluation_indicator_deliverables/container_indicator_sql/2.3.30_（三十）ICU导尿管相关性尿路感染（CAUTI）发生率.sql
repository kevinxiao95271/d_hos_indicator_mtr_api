-- 医疗安全指标 2.3.30 注册脚本（分子+分母+比率）
-- 指标编码：2.3.30 / 2.3.30.1 / 2.3.30.2
-- 指标名称：（三十）ICU导尿管相关性尿路感染（CAUTI）发生率
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.30"}';

SET @numerator_kpi_name = 'ICU患者导尿管相关性尿路感染发生例数';
SET @numerator_category_code = '2.3.30.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    'ICU患者导尿管相关性尿路感染发生例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：ICU患者导尿管相关性尿路感染发生例数；分母：同期ICU患者导尿管留置总天数。',
    'ICU患者导尿管相关性尿路感染发生例数', '无', NULL, '无',
...
SET @denominator_kpi_name = '同期ICU患者导尿管留置总天数';
SET @denominator_category_code = '2.3.30.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '同期ICU患者导尿管留置总天数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分母：同期ICU患者导尿管留置总天数。',
    '同期ICU患者导尿管留置总天数', '无', NULL, '无',
...
SET @kpi_name = '（三十）ICU导尿管相关性尿路感染（CAUTI）发生率';
SET @category_code = '2.3.30';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 4, 'decrease', 'ratio', @data_source_id, @data_source_param,
    'ICU导尿管相关性尿路感染发生率是指ICU患者导尿管相关性尿路感染发生例数占同期ICU患者导尿管留置总天数的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：ICU患者导尿管相关性尿路感染发生例数；分母：同期ICU患者导尿管留置总天数。',
    '（三十）ICU导尿管相关性尿路感染（CAUTI）发生率', '例/千导尿管日', NULL, '无',
