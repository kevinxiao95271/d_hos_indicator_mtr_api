...
SET @numerator_kpi_name = '进行Siewert分型的食管胃交界部癌患者数';
...
CALL sp_kpi_item_create(
    ...
    '进行Siewert分型的食管胃交界部癌患者数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '食管癌专病表查询', NULL, '分子：进行Siewert分型的食管胃交界部癌患者数。分母：同期食管胃交界部癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否进行Siewert分型的食管胃交界部癌', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期食管胃交界部癌患者总数';
...
CALL sp_kpi_item_create(
    ...
    '同期食管胃交界部癌患者总数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '食管癌专病表查询', NULL, '分子：进行Siewert分型的食管胃交界部癌患者数。分母：同期食管胃交界部癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期食管胃交界部癌患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = '食管胃交界部癌 Siewert 分型率';
SET @category_code = '3.19.7.4';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '食管胃交界部癌 Siewert 分型率的定义和计算方法', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '食管癌专病表查询', NULL, '分子：进行Siewert分型的食管胃交界部癌患者数。分母：同期食管胃交界部癌患者总数。', '百分比（%）', '无', NULL,
