-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.9.4.10.2（与CSV文件中的编码完全一致）
-- 指标名称：颈动脉支架置入术患者术后1年同侧缺血性卒中发生率
-- 创建日期：2026-02-05
-- ===========================================
...
-- 创建分母指标（3.9.4.10.2.2）
SET @denominator_kpi_name = '同期颈动脉支架置入术患者完成术后1年随访人数';
SET @denominator_category_code = '3.9.4.10.2.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期颈动脉支架置入术患者完成术后1年随访人数', '', '',
    '颈动脉支架置入术专病表查询', NULL, '术后随访包括电话随访、网络随访、门诊随访、再次住院。同侧缺血性卒中指靶血管供血区发生的缺血性卒中。排除：靶血管供血区发生的TIA。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期颈动脉支架置入术患者完成术后1年随访人数', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
-- 创建比率指标（3.9.4.10.2）
SET @kpi_name = '颈动脉支架置入术患者术后1年同侧缺血性卒中发生率';
SET @category_code = '3.9.4.10.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，颈动脉支架置入术患者术后30天、术后1年发生同侧缺血性卒中人数占颈动脉支架置入术患者总数的比例。', '', '',
    '颈动脉支架置入术专病表查询', NULL, '术后随访包括电话随访、网络随访、门诊随访、再次住院。同侧缺血性卒中指靶血管供血区发生的缺血性卒中。排除：靶血管供血区发生的TIA。', '百分比（%）', '无', NULL,
