...
SET @numerator_kpi_name = '首次治疗前完成临床中国肝癌分期诊断的肝细胞癌患者数';
SET @numerator_category_code = '3.19.3.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '首次治疗前完成临床中国肝癌分期诊断的肝细胞癌患者数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '肝癌专病表查询', NULL, '分子：首次治疗前完成临床中国肝癌分期诊断的肝细胞癌患者数。分母：同期接受首次治疗的肝细胞癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否首次治疗前完成临床中国肝癌分期诊断的肝细胞癌', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.19.3.1.2';
CALL sp_kpi_item_create(
...
SET @kpi_name = '肝细胞癌患者首次治疗前临床中国肝癌分期诊断率';
SET @category_code = '3.19.3.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '肝细胞癌患者首次治疗前临床中国肝癌分期诊断率的定义和计算方法', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '肝癌专病表查询', NULL, '分子：首次治疗前完成临床中国肝癌分期诊断的肝细胞癌患者数。分母：同期接受首次治疗的肝细胞癌患者总数。', '百分比（%）', '无', NULL,
