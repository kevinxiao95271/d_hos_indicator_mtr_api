...
-- 创建分子指标
SET @numerator_kpi_name = '同种异体角膜移植术后发生并发症的例数';
SET @numerator_category_code = '5.1.4.5.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同种异体角膜移植术后发生并发症（排斥反应、眼部感染、青光眼、等）的例数占同期同种异体角膜移植总例数的比例。', '《同种异体角膜移植技术临床应用管理规范（2022 版）》 《同种异体角膜移植技术临床应用质量控制指标（2022 版）》。', '反映医院同种异体角膜移植技术水平的重要指标之一。',
    '同种异体角膜移植技术数据源', NULL, '分子：是指在患者住院过程中完成同种异体角膜移植术，术后发生感染、排异反应等并发症的例数。 分母：是指同期出院患者中完成同种异体角膜移植术的总例数。', '百分比（%）', '病案首页。', NULL,
    '出院日期', '出院科室名称', '是否完成并发症发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.4.5.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期同种异体角膜移植总例数';
SET @denominator_category_code = '5.1.4.5.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成并发症发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.4.5.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '并发症发生率';
SET @category_code = '5.1.4.5';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.4.5"}]'), NULL, NULL, NULL, NULL
);
