...
SET @numerator_kpi_name = '髋关节置换术患者住院费用之和';
SET @numerator_category_code = '4.23.2.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'value', NULL, @data_source_id, @data_source_param,
    '所有髋关节置换术患者住院费用之和', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：所有髋关节置换术患者住院费用之和。分母：同期髋关节置换术出院总人数。', '无', '无', NULL, NULL,
...
SET @denominator_kpi_name = '同期髋关节置换术出院总人数_次均费用';
SET @denominator_category_code = '4.23.2.2.2';
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(
    ...
    '同期髋关节置换术出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制',
    IF(@source_kpi_id IS NOT NULL, '与指标编码4.23.1.1.1.2（同期手术患者总人数）同源，采集源于该指标', '单病种应上报资源消耗查询'), NULL, 'full', NULL, NULL, '分子：所有髋关节置换术患者住院费用之和。分母：同期髋关节置换术出院总人数。（与4.23.1.1.1.2一致）', '无', '无', NULL, NULL,
...
SET @kpi_name = '髋关节置换术次均费用';
SET @category_code = '4.23.2.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'decimal', 2, 'monitor', 'ratio', NULL, @data_source_id, @data_source_param,
    '所有髋关节置换术患者住院费用之和除以同期髋关节置换术出院总人数', '《河北省三级医院评审标准实施细则（2025年版）》第四章。', '评价病种的费用消耗情况，反映病种医疗成本控制，间接反映医疗质量',
    '单病种应上报资源消耗查询', NULL, 'full', NULL, NULL, '分子：所有髋关节置换术患者住院费用之和。分母：同期髋关节置换术出院总人数。', '元', '无', NULL, NULL,
