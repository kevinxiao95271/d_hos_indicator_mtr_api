-- 4.20.1.6 病死率（慢性阻塞性肺疾病急性发作，住院）
-- 分母 4.20.1.6.2 与 4.20.1.1.2 统计口径一致，采集源于该指标（须先执行 4.20.1.1）
...
SET @num_name = 'COPD某病种死亡人数_4.20.1.6';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '某病种的死亡人数', '无', '评价医疗质量的重要指标', '第四章-慢性阻塞性肺疾病（急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。此指标数据来自HQMS病案首页，不取自单病种质量监测平台。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '某病种的死亡人数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.20.1.6.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.20.1.6.2';
SET @source_kpi_id = (
...
SET @den_name = '同期COPD某病种出院总人数_4.20.1.6';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期某病种出院总人数', '无', '评价医疗质量的重要指标', IF(@source_kpi_id IS NOT NULL, '与指标编码4.20.1.1.2（同期慢性阻塞性肺疾病急性加重出院患者总数）同源，采集源于该指标', '第四章-慢性阻塞性肺疾病（急性发作，住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。此指标数据来自HQMS病案首页，不取自单病种质量监测平台。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期慢性阻塞性肺疾病急性加重出院患者总数', '同期某病种出院总人数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    '某病种死亡人数占同期某病种出院总人数的比例', '无', '评价医疗质量的重要指标', '第四章-慢性阻塞性肺疾病（急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。此指标数据来自HQMS病案首页，不取自单病种质量监测平台。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.20.1.6"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
