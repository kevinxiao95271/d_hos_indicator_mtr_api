...
SET @numerator_kpi_name = '早产产妇人数';
SET @numerator_category_code = '3.8.3.1.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, '{"p_date_type":"out"}',
    '早产产妇人数', '《产科专业医疗质量控制指标（2021年版）》。', '评价早产发生情况',
    '产科专病表查询', NULL, '分子：早产产妇人数。分母：同期分娩产妇总人数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否早产', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @kpi_name = '早产率';
SET @category_code = '3.8.3.1';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '单位时间内，早产（孕周在28-36+6周之间的分娩）产妇人数占同期分娩产妇（分娩孕周≥28周）总人数的比例', '《产科专业医疗质量控制指标（2021年版）》。', '评价早产发生情况',
    '产科专病表查询', NULL, '分子：早产产妇人数。分母：同期分娩产妇总人数。', '百分比（%）', '无', NULL,
