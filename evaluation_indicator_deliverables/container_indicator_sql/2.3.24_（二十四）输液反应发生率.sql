-- 医疗安全指标 2.3.24 注册脚本（分子+分母+比率）
-- 指标编码：2.3.24 / 2.3.24.1 / 2.3.24.2
-- 指标名称：（二十四）输液反应发生率
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.24"}';

SET @numerator_kpi_name = '发生输液反应的出院患者例数';
SET @numerator_category_code = '2.3.24.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '发生输液反应的出院患者例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：发生输液反应的出院患者例数；分母：同期接受输液的出院患者总人次。',
    '发生输液反应的出院患者例数', '无', NULL, '无',
...
SET @denominator_kpi_name = '同期接受输液的出院患者总人次';
SET @denominator_category_code = '2.3.24.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '同期接受输液的出院患者总人次', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分母：同期接受输液的出院患者总人次。',
    '同期接受输液的出院患者总人次', '无', NULL, '无',
...
SET @kpi_name = '（二十四）输液反应发生率';
SET @category_code = '2.3.24';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, @data_source_param,
    '输液反应发生率是指发生输液反应的出院患者例数占同期接受输液的出院患者总人次的比例。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：发生输液反应的出院患者例数；分母：同期接受输液的出院患者总人次。',
    '（二十四）输液反应发生率', '百分比（%）', NULL, '无',
