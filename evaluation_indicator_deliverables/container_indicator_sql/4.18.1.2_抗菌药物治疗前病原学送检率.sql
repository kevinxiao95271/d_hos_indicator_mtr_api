-- 4.18.1.2 抗菌药物治疗前病原学送检率（社区获得性肺炎成人）
...
SET @num_name = 'CAP成人治疗前病原学检查患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '治疗前进行病原学检查患者数', '无', '反映社区获得性肺炎治疗的规范性', '第四章-社区获得性肺炎（成人，首次住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：治疗前进行病原学检查患者数。分母：同期社区获得性肺炎患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '治疗前进行病原学检查患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.18.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期CAP成人患者总数_4.18.1.2';
-- 分母 4.18.1.2.2 源于 4.18.1.1.2，须先执行 4.18.1.1_入院氧合评估完成率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期社区获得性肺炎患者总数', '无', '反映社区获得性肺炎治疗的规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.18.1.1.2（同期社区获得性肺炎患者总数）同源，采集源于该指标', '第四章-社区获得性肺炎（成人，首次住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：治疗前进行病原学检查患者数。分母：同期社区获得性肺炎患者总数。（与4.18.1.1.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期社区获得性肺炎患者总数', '同期社区获得性肺炎患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.18.1.2.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '治疗前进行病原学检查患者数占同期社区获得性肺炎患者总数的比例', '无', '反映社区获得性肺炎治疗的规范性', '第四章-社区获得性肺炎（成人，首次住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：治疗前进行病原学检查患者数。分母：同期社区获得性肺炎患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.18.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
