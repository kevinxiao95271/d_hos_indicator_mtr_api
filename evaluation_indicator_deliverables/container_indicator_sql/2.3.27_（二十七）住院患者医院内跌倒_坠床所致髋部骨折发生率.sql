-- 医疗安全指标 2.3.27 注册脚本（分子+分母+比率）
-- 指标编码：2.3.27 / 2.3.27.1 / 2.3.27.2
-- 指标名称：（二十七）住院患者医院内跌倒/坠床所致髋部骨折发生率
-- 创建日期：2026-02
...
SET @data_source_param = '{"p_date_type":"out","p_indicator_code":"2.3.27"}';

SET @numerator_kpi_name = '住院患者发生跌倒坠床所致髋部骨折发生例数';
SET @numerator_category_code = '2.3.27.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '住院患者发生跌倒坠床所致髋部骨折发生例数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：住院患者发生跌倒坠床所致髋部骨折发生例数；分母：住院患者实际占用床日数。',
    '住院患者发生跌倒坠床所致髋部骨折发生例数', '无', NULL, '无',
...
SET @denominator_kpi_name = '住院患者实际占用床日数';
SET @denominator_category_code = '2.3.27.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'decrease', 'value', @data_source_id, @data_source_param,
    '住院患者实际占用床日数', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分母：住院患者实际占用床日数。',
    '住院患者实际占用床日数', '无', NULL, '无',
...
SET @kpi_name = '（二十七）住院患者医院内跌倒/坠床所致髋部骨折发生率';
SET @category_code = '2.3.27';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'decrease', 'ratio', @data_source_id, @data_source_param,
    '住院患者医院内跌倒/坠床所致髋部骨折发生率是指住院患者发生跌倒/坠床所致髋部骨折发生例数与住院患者实际占用床日数的千分比。', '《2.3 医疗安全指标列表.csv》。', '反映医院围手术期/医疗安全管理质量',
    '医疗安全指标明细查询', NULL, 'full', NULL, '无',
    '分子：住院患者发生跌倒坠床所致髋部骨折发生例数；分母：住院患者实际占用床日数。',
    '（二十七）住院患者医院内跌倒/坠床所致髋部骨折发生率', '千分比（‰）', NULL, '无',
