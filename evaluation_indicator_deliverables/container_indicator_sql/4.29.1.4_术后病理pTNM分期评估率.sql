-- 4.29.1.4 术后病理pTNM分期评估率（肺癌，手术治疗）
-- 分母 4.29.1.4.2 与 4.29.1.1.2 统计口径一致，采集源于该指标（须先执行 4.29.1.1）
...
SET @num_name = '肺癌术后病理pTNM分期评估例数_4.29.1.4';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '肺癌术后病理pTNM分期评估例数', '无', '反映肺癌术后病理评估规范性', '第四章-肺癌（手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：肺癌术后根据病理诊断报告进行pTNM分期评估例数。分母：同期手术患者例数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '肺癌术后病理pTNM分期评估例数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.29.1.4.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.29.1.4.2';
SET @source_kpi_id = (
...
SET @den_name = '同期手术患者例数_4.29.1.4';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期手术患者例数', '无', '反映肺癌术后病理评估规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.29.1.1.2（同期该肿瘤手术患者总数）同源，采集源于该指标', '第四章-肺癌（手术治疗）质量控制查询'), NULL, 'full', NULL, NULL, '分子：肺癌术后病理pTNM分期评估例数。分母：同期手术患者例数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期该肿瘤手术患者总数', '同期手术患者例数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
SET @kpi_name = '术后病理pTNM分期评估率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '肺癌术后病理pTNM分期评估例数占同期手术患者例数的比例', '无', '反映肺癌术后病理评估规范性', '第四章-肺癌（手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：肺癌术后根据病理诊断报告进行pTNM分期评估例数。分母：同期手术患者例数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.29.1.4"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
