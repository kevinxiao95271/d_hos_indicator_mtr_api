-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.14.1.8（与CSV文件中的编码完全一致）
-- 指标名称：急性STEMI患者住院期间应用超声心动图（UCG）评价左心室射血分数（LVEF）的比例
-- 创建日期：2026-02-04
-- ===========================================
...
-- 创建分子指标（3.14.1.8.1）
SET @numerator_kpi_name = '住院期间通过UCG评价LVEF的急性STEMI患者数';
SET @numerator_category_code = '3.14.1.8.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '住院期间通过UCG评价LVEF的急性STEMI患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价STEMI急性期规范化诊疗与评估情况',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：住院期间通过UCG评价LVEF的急性STEMI患者数。分母：同期急性STEMI患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否住院期间通过UCG评价LVEF', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.1.8.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '急性STEMI患者总数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价STEMI急性期规范化诊疗与评估情况',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：住院期间通过UCG评价LVEF的急性STEMI患者数。分母：同期急性STEMI患者总数。', '无', '无', NULL,
...
-- 创建比率指标（3.14.1.8）
SET @kpi_name = '急性STEMI患者住院期间应用超声心动图（UCG）评价左心室射血分数（LVEF）的比例';
SET @category_code = '3.14.1.8';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '住院期间通过UCG评价LVEF的急性STEMI患者数，占同期急性STEMI患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价STEMI急性期规范化诊疗与评估情况',
    '急性ST段抬高型心肌梗死专病表查询', NULL, '分子：住院期间通过UCG评价LVEF的急性STEMI患者数。分母：同期急性STEMI患者总数。', '百分比（%）', '无', NULL,
