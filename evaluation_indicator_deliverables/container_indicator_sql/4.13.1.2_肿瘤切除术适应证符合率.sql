-- 4.13.1.2 肿瘤切除术适应证符合率（胶质瘤）
-- 分母 4.13.1.2.2 与 4.13.1.1.2 统计口径一致，采集源于该指标（须先执行 4.13.1.1）
...
SET @num_name = '胶质瘤符合肿瘤切除术适应证患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    '符合肿瘤切除术适应证的患者数', '无', '反映胶质瘤手术诊疗的规范性', '第四章-胶质瘤（初发，手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：符合肿瘤切除术适应证的患者数。分母：同期行胶质瘤切除术的患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '符合肿瘤切除术适应证的患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.13.1.2.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.13.1.2.2';
SET @source_kpi_id = (
...
SET @den_name = '同期胶质瘤切除术患者总数_4.13.1.2';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期行胶质瘤切除术的患者总数', '无', '反映胶质瘤手术诊疗的规范性', IF(@source_kpi_id IS NOT NULL, '与指标编码4.13.1.1.2（同期行胶质瘤活检术的患者总数）同源，采集源于该指标', '第四章-胶质瘤（初发，手术治疗）质量控制查询'), NULL, 'full', NULL, NULL, '分子：符合肿瘤切除术适应证的患者数。分母：同期行胶质瘤切除术的患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期行胶质瘤活检术的患者总数', '同期行胶质瘤切除术的患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
SET @kpi_name = '肿瘤切除术适应证符合率';
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    '符合肿瘤切除术适应证的患者数占同期行胶质瘤切除术的患者总数的比例', '无', '反映胶质瘤手术诊疗的规范性', '第四章-胶质瘤（初发，手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：符合肿瘤切除术适应证的患者数。分母：同期行胶质瘤切除术的患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.13.1.2"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
