...
-- 专病表接口指标注册脚本
-- 指标编码：3.19.10.2
-- 指标名称：肾癌患者首次治疗前临床TNM分期检查评估策略符合率
-- 创建日期：2026-03-09
-- ===========================================

SET @data_source_id = (SELECT ID FROM DATASOURCE WHERE DATASOURCE_NAME = '肾癌专病表查询' AND INVALID = 0 LIMIT 1);
SET @category_id = (SELECT ID FROM KPI_CATEGORY WHERE CATEGORY_NAME LIKE '%肿瘤%' AND INVALID = 0 LIMIT 1);
SET @create_user = 'system';

SET @numerator_kpi_name = '首次治疗前临床TNM分期检查评估符合策略的肾癌患者数';
SET @numerator_category_code = '3.19.10.2.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '首次治疗前临床TNM分期检查评估符合策略的肾癌患者数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '肾癌专病表查询', NULL, '分子：首次治疗前临床TNM分期检查评估符合策略的肾癌患者数。分母：同期接受首次治疗的肾癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否首次治疗前临床TNM分期检查评估符合策略的肾癌', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_kpi_name = '同期接受首次治疗的肾癌患者总数';
SET @denominator_category_code = '3.19.10.2.2';
CALL sp_kpi_item_create(
    @denominator_kpi_name, @category_id, NULL, 'integer', 0, 'increase', 'value', @data_source_id, '{"p_date_type":"out"}',
    '同期接受首次治疗的肾癌患者总数', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '肾癌专病表查询', NULL, '分子：首次治疗前临床TNM分期检查评估符合策略的肾癌患者数。分母：同期接受首次治疗的肾癌患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否同期接受首次治疗的肾癌患者', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
...
SET @kpi_name = '肾癌患者首次治疗前临床TNM分期检查评估策略符合率';
SET @category_code = '3.19.10.2';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'increase', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '肾癌患者首次治疗前临床TNM分期检查评估策略符合率的定义和计算方法', '《肿瘤专业医疗质量控制指标（2023版）》。', '评价肿瘤专业诊疗规范性。',
    '肾癌专病表查询', NULL, '分子：首次治疗前临床TNM分期检查评估符合策略的肾癌患者数。分母：同期接受首次治疗的肾癌患者总数。', '百分比（%）', '无', NULL,
