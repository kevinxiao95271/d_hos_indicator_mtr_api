-- 4.33.1.5 术后非计划再手术率（结肠癌）
-- 分母 4.33.1.5.2 与 4.33.1.1.2 统计口径一致，采集源于该指标（须先执行 4.33.1.1）
...
SET @num_name = '结肠癌非计划再手术患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '病种非计划再手术的患者数', '无', '评价手术质量', '第四章-结肠癌（手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：病种非计划再手术的患者数。分母：同期病种的患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '病种非计划再手术的患者数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.33.1.5.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.33.1.5.2';
SET @source_kpi_id = (
...
SET @den_name = '同期结肠癌病种患者总数_4.33.1.5';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期病种的患者总数', '无', '评价手术质量', IF(@source_kpi_id IS NOT NULL, '与指标编码4.33.1.1.2（同期该肿瘤手术患者总例数）同源，采集源于该指标', '第四章-结肠癌（手术治疗）质量控制查询'), NULL, 'full', NULL, NULL, '分子：病种非计划再手术的患者数。分母：同期病种的患者总数。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期该肿瘤手术患者总例数', '同期病种的患者总数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    '病种非计划再手术的患者数占同期病种患者总数的比例', '无', '评价手术质量', '第四章-结肠癌（手术治疗）质量控制查询', NULL, 'full', NULL, NULL, '分子：病种非计划再手术的患者数。分母：同期病种的患者总数。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.33.1.5"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
