-- ===========================================
-- 专病表接口指标注册脚本
-- 指标编码：3.14.4.4（与CSV文件中的编码完全一致）
-- 指标名称：原发性醛固酮增多症确诊试验开展率
-- 创建日期：2026-02-04
-- ===========================================
...
-- 创建分子指标（3.14.4.4.1）
SET @numerator_kpi_name = '住院期间接受确诊试验检查的原发性醛固酮增多症患者数';
SET @numerator_category_code = '3.14.4.4.1';
CALL sp_kpi_item_create(
    @numerator_kpi_name, @category_id, NULL, 'integer', 0, 'monitor', 'value', @data_source_id, '{"p_date_type":"out"}',
    '住院期间接受确诊试验检查的原发性醛固酮增多症患者数', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院对原发性醛固酮增多症诊治的规范性',
    '高血压专病表查询', NULL, '分子：住院期间接受确诊试验检查的原发性醛固酮增多症患者数。确诊试验包括卡托普利试验、生理盐水输注试验、口服高钠饮食、氟氢可的松试验。分母：同期原发性醛固酮增多症住院患者总数。', '无', '无', NULL,
    '出院日期', '出院科室名称', '是否住院期间接受确诊试验检查_原发性醛固酮增多症', NULL, NULL, NULL, NULL, 1.0000, 'sum',
...
SET @denominator_category_code = '3.14.4.4.2';
CALL sp_kpi_item_create(
...
-- 创建比率指标（3.14.4.4）
SET @kpi_name = '原发性醛固酮增多症确诊试验开展率';
SET @category_code = '3.14.4.4';
CALL sp_kpi_item_create(
    @kpi_name, @category_id, NULL, 'percent', 2, 'monitor', 'ratio', @data_source_id, '{"p_date_type":"out"}',
    '住院期间接受确诊试验检查的原发性醛固酮增多症患者数，占同期原发性醛固酮增多症住院患者总数的比例', '《心血管系统疾病相关专业医疗质量控制指标（2021年版）》。', '评价医院对原发性醛固酮增多症诊治的规范性',
    '高血压专病表查询', NULL, '分子：住院期间接受确诊试验检查的原发性醛固酮增多症患者数。确诊试验包括卡托普利试验、生理盐水输注试验、口服高钠饮食、氟氢可的松试验。分母：同期原发性醛固酮增多症住院患者总数。', '百分比（%）', '无', NULL,
