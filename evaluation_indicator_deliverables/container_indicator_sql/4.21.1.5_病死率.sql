-- 4.21.1.5 病死率（哮喘成人，急性发作，住院）
-- 分母 4.21.1.5.2 与 4.21.1.1.2 统计口径一致，采集源于该指标（须先执行 4.21.1.1）
...
SET @num_name = '哮喘成人某病种死亡人数_4.21.1.5';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'decrease', 'value', NULL, @ds_id, @param,
    '某病种的死亡人数', '无', '评价医疗质量的重要指标', '第四章-哮喘（成人，急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。此指标数据来自HQMS病案首页，不取自单病种质量监测平台。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', '某病种的死亡人数', NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.21.1.5.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @denominator_category_code = '4.21.1.5.2';
SET @source_kpi_id = (
...
SET @den_name = '同期某病种出院总人数_4.21.1.5';
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id), IF(@source_kpi_id IS NOT NULL, NULL, @param),
    '同期某病种出院总人数', '无', '评价医疗质量的重要指标', IF(@source_kpi_id IS NOT NULL, '与指标编码4.21.1.1.2（同期支气管哮喘急性发作出院患者总例数）同源，采集源于该指标', '第四章-哮喘（成人，急性发作，住院）质量控制查询'), NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。此指标数据来自HQMS病案首页，不取自单病种质量监测平台。', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '同期支气管哮喘急性发作出院患者总数', '同期某病种出院总人数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'decrease', 'ratio', NULL, @ds_id, @param,
    '某病种死亡人数占同期某病种出院总人数的比例', '无', '评价医疗质量的重要指标', '第四章-哮喘（成人，急性发作，住院）质量控制查询', NULL, 'full', NULL, NULL, '分子：某病种的死亡人数。分母：同期某病种出院总人数。此指标数据来自HQMS病案首页，不取自单病种质量监测平台。', '百分比（%）', '无', NULL, NULL,
    '出院日期', '出院科室名称', NULL, @num_id, @den_id, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.21.1.5"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
