...
SET @numerator_category_code = '5.1.2.5.1';
CALL sp_kpi_item_create(
    ...
    '同种胰岛移植技术数据源', NULL, '分子：是指在患者住院过程中完成同种异体运动系统结构性组织移植术，术后30 天内发生感染、血栓形成等并发症的例次数。 分母：是指同期出院患者中完成同种异体运动系统结构性组织移植术的患者总例次数。', '百分比（%）', '病案首页。病历资料、不良事件报告管理系统。', NULL,
    '出院日期', '出院科室名称', '是否完成围手术期并发症发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.2.5.1"}]'), NULL, NULL, NULL, NULL
);
SET @numerator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @numerator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建分母指标
SET @denominator_kpi_name = '同期同种胰岛移植治疗总例次数';
SET @denominator_category_code = '5.1.2.5.2';
CALL sp_kpi_item_create(
    ...
    '出院日期', '出院科室名称', '是否完成围手术期并发症发生', NULL, NULL, NULL, NULL, 1.0000, 'sum',
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.2.5.2"}]'), NULL, NULL, NULL, NULL
);
SET @denominator_kpi_id = (SELECT ID FROM KPI_ITEM WHERE KPI_NAME = @denominator_kpi_name AND INVALID = 0 AND KPI_TYPE = 'value' ORDER BY CREATE_TIME DESC LIMIT 1);

-- 创建比率指标
SET @kpi_name = '围手术期并发症发生率';
SET @category_code = '5.1.2.5';
CALL sp_kpi_item_create(
    ...
    '["出院科室名称","入院科室名称"]', '[]', '["患者ID","住院号","患者姓名","病案号","出院时间"]',
    @create_user, CONCAT('[{"category_id":', @category_id, ',"category_code":"5.1.2.5"}]'), NULL, NULL, NULL, NULL
);
