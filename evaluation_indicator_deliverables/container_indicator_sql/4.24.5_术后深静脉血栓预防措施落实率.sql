...
SET @num_name = '髋关节置换术进行深静脉血栓预防患者数';
CALL sp_kpi_item_create(@num_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, @ds_id, @param,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.24.5.1"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
SET @den_name = '同期髋关节置换术手术患者总例数_4.24.5';
-- 分母 4.24.5.2 源于 4.24.3.2，须先执行 4.24.3_髋膝关节置换术单侧手术输血量小于400ml的比率.sql
SET @source_kpi_id = (
...
CALL sp_kpi_item_create(@den_name, @cat_id, NULL, 'integer', 0, 'increase', 'value', NULL, IF(@source_kpi_id IS NOT NULL, NULL, @ds_id),
    ...
    '同期手术患者总例数', '无', '反映患者VTE预防情况', IF(@source_kpi_id IS NOT NULL, '与指标编码4.24.3.2（4.24.3.2）同源，采集源于该指标', '第四章-髋关节置换术质量控制查询'), NULL, 'full', NULL, NULL, '分子：单位时间进行深静脉血栓预防的患者数。分母：同期手术患者总例数。（与4.24.3.2一致）', '无', '无', NULL, NULL,
    '出院日期', '出院科室名称', IF(@source_kpi_id IS NOT NULL, '4.24.3.2', '同期手术患者总例数'), NULL, NULL, NULL, NULL, 1.0000, 'sum', '["出院科室名称"]', '[]', '["患者ID","住院号","病案号","出院时间"]',
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.24.5.2"}]'), NULL, NULL, IF(@source_kpi_id IS NOT NULL, 'datasource', NULL), NULL, @source_kpi_id, NULL, 1);
...
CALL sp_kpi_item_create(@kpi_name, @cat_id, NULL, 'percent', 2, 'increase', 'ratio', NULL, @ds_id, @param,
    ...
    @user, CONCAT('[{"category_id":', @cat_id, ',"category_code":"4.24.5"}]'), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
