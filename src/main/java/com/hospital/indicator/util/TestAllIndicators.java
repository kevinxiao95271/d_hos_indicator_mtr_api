...
/**
 * Test all indicators that have related_items
 */
public class TestAllIndicators {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Query indicators that have related_items (not null and not empty)
            String sql = "SELECT metric_code, metric_name, related_items FROM t_indicator WHERE status = 1 AND related_items IS NOT NULL AND related_items != '' AND related_items != 'null' ORDER BY metric_code";

            ResultSet rs = stmt.executeQuery(sql);

            System.out.println("========================================");
            System.out.println("Testing All Indicators with related_items");
            System.out.println("========================================\n");
            ...
            while (rs.next()) {
                ...
                System.out.print("[" + totalCount + "] " + name + " ... ");

                int deptCount = testIndicator(code);

                if (deptCount > 0) {
                    successCount++;
                    System.out.println("SUCCESS (" + deptCount + " depts)");
                } else {
                    failCount++;
                    System.out.println("FAILED (0 depts)");
                }
            }
            rs.close();
